...
▶ **ACTIVE: `S1.12.4` — DISPATCH PASS 5. THE ROUTE AND THE BRIEF ARE READY; 🎯 2026-08-29 ASKED FOR A
FRESH SESSION TO RUN IT.** ⛔ **Do not dispatch from a session that has been fixing** — `audits always use
fresh agents` is the rule, and the incumbent re-reads its own premises.

**Everything the run needs is on disk in `docs/audits/2026-08-29-s1-money-pass5/`:**

| | |
|---|---|
| `BRIEF.md` | the five reading rules, **and what pass 4 measured about its own findings** |
| `RESUME-PROTOCOL.md` | generated by the route — incremental writes · heap **1536 MB** · ⛔ an OOM is a **finding** |
| `ROUTING-{A,B,C,D}.txt` | the lane manifests — **A 108 · B 113 · C 122 · D 50** |
| `ROUTING-ORIGINS.tsv` | every file's origin, so [D69]'s exemption is a lookup rather than a judgement |

⚠️ **Dispatch shape:** four lane agents, ⛔ **no sub-agents**, each writing `<lane>-*.md` **as it goes** —
pass 4 lost three auditors mid-round and kept 11 findings only because they were already on disk.

⚡ **393 routed · 0 unrouted · 0 owed**, and **207 `neighbour` + 49 `s0-first-look` are in a lane for the
first time** — a file that did not change could not be routed at all until `S1.11.6`. ⚠️ Expect most
findings to be `first-look` or `neighbour`: [D69] says those do **not** restart the convergence count, and
⛔ **exempt from the count is not exempt from the fix.**
