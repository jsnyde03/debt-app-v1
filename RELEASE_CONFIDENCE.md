...
| Subscription/tier gating (`lib/subscription/hasFeatureAccess.ts`) | **Fixed this pass** (was Low) | Zero tests existed anywhere for this function. Notifications gating in `app/page.tsx` bypassed it entirely via raw `subscriptionPlan === "premium"` string checks instead of calling `hasFeatureAccess`. The `interest_savings` `PremiumFeature` was declared but never actually gated anywhere. Fixed: notifications now route through `hasFeatureAccess`; `interest_savings` removed (it described a UI surface that doesn't independently exist — it's covered by the `strategy_comparison` gate); added a regression test matrix asserting free vs. premium access per feature. |
