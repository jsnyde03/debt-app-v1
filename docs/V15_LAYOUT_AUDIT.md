...
- **2.15.1 — Cross-cutting CSS correctness (do first; clears noise before per-surface work).**
  - Fix the two silently-dropped `calc()` paddings in `08-dark-theme-polish.css`: `evn(`→`env(` (:757) and the missing space `+7rem`→`+ 7rem` (:438). ⚠️ These were *dropped* (invalid calc), so fixing them **applies** the intended scroll-runway — correct on phone (clears the floating bottom nav) but **dead space on iPad** (no floating nav), so scope the runway to phone as part of the fix.
