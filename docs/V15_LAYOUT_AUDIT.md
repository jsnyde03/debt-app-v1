...
| Consistency sweep (2.15.8) | 🔄 | 🔄 | **Full-suite regression: 114 passed** across all 4 projects — every 2.15 fix keeps the suite green. The prior 1 hard failure (`planner-herdening.spec.ts:49` old-seed race) is **fixed in 2.15.9** — the next full regression should be clean 4-project green. Remaining sweep: Living-Expenses column rebalance, dedup `.center-modal.debt-add-modal`, secondary-modal spot-checks. | 🔄 |
