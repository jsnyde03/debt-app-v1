...
- **Journey hero — decided: milestones ON the ring** (Jason 2026-07-21), green→gold arc, count-up, glow; a **status display** (interactivity deferred → Phase 3 tappable-milestone detail).
  - **✅ WIRED + both-theme web-verified (2026-07-21) — pending Jason sign-off.** `progress.tsx` hero now holds `<JourneyRingCanvas>` (green→gold SweepGradient arc + GPU glow, sweep 0→pct on mount; 25/50/75/Free nodes ON the arc: passed=green, next/Free=gold-glow) + a centered `<CountUp>` % overlay + the DEBT-FREE date/paid meta beside it. Milestone states derived from `pct` (`next` = first unreached; `100` always the gold destination at 12 o'clock). Palette = **constants** (dark-scheme green/gold) since the navy panel is theme-invariant. Ring a11y collapsed to one groupLabel (pct + reached/next milestones + debt-free date); the CountUp overlay is SR-hidden (`no-hide-descendants`). Verified at 0% and ~55%: nodes read clearly on the arc → **the rail-in-panel fallback was NOT needed.** tsc 0; 0 console errors both themes. **On-ring readability confirmed → the separate `<MilestonesRow>` is removed from the hero.**
  - **Dead files (unused, no other importers): `ProgressRing.tsx` (old SVG ring) + `MilestonesRow.tsx` (rail).** Kept in-tree pending Jason's aesthetic sign-off on the on-ring treatment; **delete both on sign-off.**
  - **Surfaced during wiring (after-scan):** (1) MomentumStats "Paid so far" now openly duplicates the hero's "$X of $Y paid" → the **P3 de-dupe** below is the immediate next step. (2) Demo "Interest saved" reads $0.00 even at 55% (no captured extra-payment history in the seed) — a demo-data artifact, not a ring bug; worth a glance when P3 reworks the momentum metric. (3) The SweepGradient can't literally warm 0%→100% along the arc (start + Free share 12 o'clock) — reads well aesthetically; a more-literal along-arc gradient is optional polish. (4) `next`-node color is gold (per the built design); the rail used blue — a small choice for Jason at sign-off.
  - **✅ P3 RESOLVED — momentum FOLDED INTO the trajectory, not kept as a card (Jason 2026-07-21).** Jason's call: swapping "Paid so far" for "Time saved" still left it "just text" and redundant with the trajectory. So the whole `MomentumStats`/`MomentumCard` was **cut**, and the "beats minimums" story became a **felt visual** on the trajectory chart: the ghost line is now the **minimum-payments baseline** (`buildPayoffTrajectory` extra=0 — pays off later, or trails high when interest outruns minimums) instead of the other strategy, with a **labeled legend** ("— Your plan" / "— Minimum payments") + the win in green ("$1,222 · 14 months saved"). Shown only when there's a gap (`saving`/`payoff-enabling`); the `none` state renders the plain trajectory, no ghost. New `payoffSelectors.minimums`; `TrajectoryChart` takes `minimums` + `interestSaved`. Both-theme web-verified (saving + none); tsc 0; 0 console errors. Removes the redundancy the audit flagged (two "payoff over time vs alternative" elements → one). _After-scan: (1) **`payoff-enabling` not visually verified** (couldn't cheaply seed a store where minimums can't cover interest) — code path simple ("Minimums never pay it off" + a high-trailing ghost); flag → device/edge verify. (2) The "Debt-free {date}" footer label is right-aligned while the gold plan-endpoint bead sits mid-chart when the ghost extends past it — minor caption/anchor disconnect, optional polish. (3) Ghost at 0.3 opacity reads but is subtle; legend carries the meaning. (4) The demo sits in the EF-phase `none` state → it shows the WEAK version of every Progress element (ring 0%, no ghost) → **demo-data overhaul is the next task (Jason 2026-07-21).**_
  - Then: **P2 Skia cushion bars**. Target: **hero → cash-flow → trajectory(+vs-minimums)**.
