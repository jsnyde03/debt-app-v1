...
These 33 are not "hard", they are structurally out of reach, and no amount of lane investment moves
them:
...
| **iPad hardware input** | §10's ⌘N · ⌘1/2/3 · ⌘ HUD · pointer hover (5 checks) | `pressKey` has no modifier chords; there is no pointer. |
...
**What this does not change:** the device pass still has to happen. 33 checks are permanently a
