...
| `D2-4c` | ⚠️ **REFUTED AS FILED, 2026-09-03 — the mechanism was mine and it was wrong** *(class-1 re-audit 3 · `T13`)*. A fresh auditor measured that a mid-batch fault DOES restore the earlier plant and that batch verdicts equal solo verdicts, so **`[D77]` needs no change on this evidence**. ⛔ The two OBSERVATIONS stand — two files were genuinely left modified, one of them production code — and the mechanism is confirmed reachable only under an artificial `fault()` inside the plant window, which no current call site occupies. **Law IV on my own finding: the observation was real, the cause I assigned was not.** Original text follows. ⛔ **A `prove:guards` BATCH THAT FAULTS MID-RUN LEAVES ITS PLANT IN PRODUCTION CODE.** Found 2026-09-02 while re-proving the guards that went stale at class 1's boundary. A batch faulted on `S1-ROUTE-EXIT-REACHABLE` and **`apps/rn/src/data/readBackup.ts` was left holding the un-fix**: `describeRestorePreview` without `${describeLosses(store)}` — the loss disclosure a user reads **before an irreversible restore**. ⚡ **A commit at that moment ships it.** The harness does say `⚠️ this run left 1 other file(s) modified`, but only on the NEXT run, and only if one happens. ⚠️ It also means a fault can silently invalidate later verdicts in the same batch, since the tree is no longer the tree they assume. ⛔ **MEASURED TWICE, in two files, and the second is an instrument**: a later run left `scripts/check-runner-completeness.ts` holding `const missing: string[] = []`, which makes that gate **vacuous** — it would report every test file wired, forever. One left a false statement in front of a user; the other left a gate that cannot fail. ⚡ **And a batch is not equivalent to its members**: `S1P3-C7-LOSSES` scored `control=exit 1` inside a batch and `control=exit 0 · MATCHED` run alone, so a fault does not merely stop the run — it corrupts the verdicts around it |
