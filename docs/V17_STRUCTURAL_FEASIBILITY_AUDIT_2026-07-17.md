...
|---|---|---|
| **D1** | **Re-cut v1.7?** | **YES — split.** `v1.7` = "turn on the till + carrot" (Phase 3 monetization incl. Sentry/PostHog + B1; Phase 2 Drift + calendar + momentum; only feature-coupled Phase-1 robustness — §1.1 money, §1.2 types, §1.3 **SnowballSection only**, §1.5 perf, §0.3 Drift recording seam, tsc-in-CI; Phase 4 **date-correctness §4.1**; Phase 7 gate). A new **`v1.7.5` "robust foundation"** (ResultsSection extraction + full §1.6 CI hardening + §1.4 Tailwind drop + §1.7 backups + Phase 5 bugs + Phase 6 polish) ships **before Android v1.8** — the cut-seam is an interstitial version, **never v1.8.** |
