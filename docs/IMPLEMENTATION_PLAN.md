...
4. **`LivingExpensesSection` placement (shipped state):** `LivingExpensesSection` is rendered *above* the `bills-expenses-col` / `bills-debts-col` layout in JSX — not inside either column. This means it is always visible on the bills tab regardless of which sub-tab (`billsView`) is active, and on iPad it spans the full width above the two-column grid. This placement also prevents the floating add button (FAB) from overlapping the "Manage" button header: since Living Expenses is the first card in the bills tab, the Manage button is visible before the user scrolls (and therefore before the FAB appears).
