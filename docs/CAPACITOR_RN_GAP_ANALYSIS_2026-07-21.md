...
3. **✅ `expenseType` fixed/variable DONE (2026-07-21)** — `ExpenseSheet` now captures it (a "Variable amount (estimate)" switch) and includes it in add/edit; the bill row shows "· Variable" for variable bills. (Type already had the field via the `@core` re-export — census was half-right; the form was the gap.) Verified nothing consumes it functionally → data-completeness + user signal, not a calc bug. tsc 0; row verified (dark; token-only text suffix on the theme-verified ListRow). _After-scan: the presets picker that auto-sets it is a separate deferred gap. Nothing else surfaced._
4. **✅ In-app review gating DONE (2026-07-21)** — added a persisted `reviewPrompted` flag (`DebtStore` + `markReviewPrompted()` action); the `onCapture` caller now fires `maybeRequestReview()` **once** (`cycleHistory>=2 && !reviewPrompted`) instead of every qualifying capture. tsc 0; logic-verified (native prompt = device-QA). _After-scan: flag lives in the store blob → reset/fresh install can re-ask once (acceptable). Nothing else surfaced._
5. **✅ Windfall flow DONE (2026-07-21)** — Jason: on the hero, done correctly. A per-cycle `windfall` field: `selectAllocation` adds it on top of the recurring paycheck, `setWindfall` sets it, rollover clears it (never inflates the recurring amount — corrects Capacitor's permanent bump). Hero affordance: "⊕ Add extra income" → gold "⊕ $X extra this cycle" when set (opens `WindfallSheet`); the extra flows to Free. Both themes verified; tsc 0. _After-scan: flows to Free correctly; nothing else surfaced._

**✅ REBUILD-NOW BATCH COMPLETE (2026-07-21):** Timeline · interest-saved · expenseType · review-guard · windfall. _Phase after-scan: no cross-item conflicts; all uncited oversights closed. Still open (below) + P2 Skia cushion bars + Progress P1 ring hero (pre-detour priority)._
