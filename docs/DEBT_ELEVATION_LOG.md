...
### ⭐ A new verdict letter, because none of the existing ones was true

The taxonomy is a set of TOOLS: `M` Maestro (68) · `M◐` its automatable half (26) · `A` Appium (3 —
exactly the ⌘-key rows §10.5/§10.6/§10.7) · `D` device-only · `—` not a check. Springboard rows could
never become `[M]`: Maestro genuinely cannot see outside the app under test, which is the stated reason
they were `[D]` in the first place. **`[X]` = a native driver can carry it.**

⚠️ **`X` is a VERDICT, not a STATUS** — the axis this instrument is built on. Re-verdicting moves rows out
of the device pass and into *coverable, not yet built*. It adds nothing to *covered*, and the numbers
confirm it did not:

| | before | after |
|---|---:|---:|
| covered — PROVEN | 24 | **24** *(unchanged, by design)* |
| coverable, not yet built | 64 | **74** |
| permanently device-owed | 34 | **24** |
| **🎯 the device pass** | 60 | **50** |

### ⛔ "~16 rows" was too broad, and the row-by-row check IS the item

The plan had claimed ~16 rows become reclassifiable. Reading all sixteen against **what the probe
actually proved** — springboard reachable AND inspectable, nothing more — only **ten** qualify: they need
to *read or tap* springboard content (§5.2 · §5.5 · §5.6 · §5.7 · §6a.1 · §6a.3 · §6a.4 · §6a.5 · §6a.8 ·
§6b.3). Five stay `[D]`:

- ⛔ **§5.4 StandBy — PERMANENT.** *"put the phone **on a charger**, landscape"* is physical state a
  simulator has no concept of. Springboard reach is irrelevant to it, and no probe will change that.
- **§5.1** widget gallery · **§5.3** Lock-Screen Customize · **§6a.2** Island long-press (needs a 15/16 Pro
  sim) · **§10.3** Stage Manager drag-to-resize — each needs **its own probe**; reach does not imply any
  of them.

⚡ **Moving all sixteen would have booked StandBy as automatable** — the identical overstatement 4.1.9c
was built to eliminate, one layer up and in a document nobody re-reads. *Reach is not coverage*, and it
turns out reach is not even reach for five of these.

### ✅ [DECISION] 4.1.9 — XCUITest, and no Appium

Appium's entire unique value was **three** checks (measured, not estimated). XCUITest already delivers
what those need plus the ten rows above plus four of the six premium-a11y bullets, on the simulator this
lane already boots — against a second driver, a second language and a server process. **Settled: no
Appium.**

⚠️ **The three `[A]` rows stay `[A]`, not `[X]`.** XCUITest's `typeKey(_:modifierFlags:)` is unproven
here, and re-verdicting them on expectation is precisely the move this item just refused for StandBy.
▶ They need a ⌘-key probe to become `[X]`, or they fall to `[D]`. ⚠️ **Filed:** with Appium declined,
`[A]` now names a tool the project will not build — the letter is a placeholder for that probe's answer,
not a plan.

### ⭐ [.7.4h fold-in] `findings=0` now has to mean something

The audit returned 0 findings on all four types. Plausible — but `.runningForeground` means the process
is frontmost, **not** that React has rendered, so "audited and clean" and "audited an empty screen" were
indistinguishable. The probe now waits for a known element (`tab-today`, verified to be a real
`tabBarButtonTestID`) and prints `PROBE a11yAnchor id=tab-today rendered=<bool> elements=<n>` before
auditing. ⚠️ **Reported, not asserted** — if the anchor never appears the audit numbers are untrustworthy,
and saying so is more useful than reddening a probe whose job is to report. Rides the next dispatch.

### ⚠️ [.7.5 after-scan] The plan quoted the old figures, and had before

`4.1.6a`'s summary still read *"34 · 63 · 34; the device pass is 60 rows"* — stale twice over now (4.1.9c
moved it, then this did). Corrected, and the line now says to read the numbers from the generated file
rather than from any doc quoting them. **A number copied into prose is a number that goes stale
silently**, and `coverage-split.md` is regenerated by a command precisely so it cannot.

---

## 🔚 SESSION CLOSE 2026-08-17 — read this first next session

**Everything committed and pushed to `v1.7-dev`; tree clean; nothing in flight; last three native runs
all green.** Two live tracks, precedence set by 🎯: **4.1 first, 3.5.7 fills its waits.**

### ⭐ What exists now that did not at the start

| | |
|---|---|
| **The XCUITest target** | builds, runs, and **springboard reach is PROVEN** (`reachable=true elements=241`) — the single stated reason ~16 rows were device-only |
| **The a11y audit** | **mapped, not guessed**: `contrast` 0.8s · `hitRegion` 0.2s · `textClipped` 1.6s · `trait` 0.3s, all complete, vs `.all` timing out at 47.7s |
| **The suites** | iPhone **10/10** (first time) · iPad **5/5** (first content ever) |
| **⚡ The fast path** | `.app` cache + `hermesc` both worked for the first time: **43m33s → 20m57s** |
| **The fast lane** | `scope=xcuitest` — build + probe only, ~45 → ~20 min |
| **Coverage** | **24 proven · 9 automatable-half · 74 coverable-not-built · 24 device-owed**; the device pass **60 → 50** |
| **3.5.7** | `.1`–`.4` done: Slider web a11y · `.2` collapsed · `sessionStorage` behind a build flag · the **privacy gate**, non-vacuity proven |

### ▶ 4.1.9b — decomposed here, per the one-section rule (retrieve at switch-in)

1. **Extract the build recipe into a composite action** — `app-preview.yml` duplicates it and lacks the cache.
2. **Tier split** — `build → [iphone ‖ ipad]`, sum → max. ⛔ **Do it for the ~11 minutes, NOT for flakiness**: the recorded stall hit the **iPhone** tier while the iPad tier came up fine, and the 2026-08-14 iPad stall was a 240 s timeout, now fixed at parity.
3. ⛔ **Keep the XCUITest probe step in the iPhone job** — it currently sits between the two tiers; a split that drops it looks exactly like a probe that found nothing.
4. ⭐ **Cache the `.xctestrun` + `-Runner.app` beside the `.app`** — this is what makes probe iteration stop costing a full rebuild. Today every probe fix touches `apps/rn/plugins/**`, which is in the cache key.
5. **A durable per-flow results file** — 4.1.9c's writer needs it (artifacts expire), so the workflow is touched once.
6. **Upload the failure hierarchy as its own small artifact** — diagnosing flow 10 needed a 48 MB/122 MB pull to read one string; the deciding file is 72 KB.

### ⛔ Do NOT build on these — each was measured wrong once already

- ⛔ **"Reach is coverage."** Springboard being reachable does not make the widget gallery, Lock-Screen Customize, Island long-press or Stage Manager drag drivable — **none was probed**. And **§5.4 StandBy is permanently device-owed**: *"on a charger"* is physical state a simulator has no concept of.
- ⛔ **`[A]` names a tool the project declined.** The three ⌘-key rows need a `typeKey` probe to become `[X]`, or they fall to `[D]`. Do not move them on expectation.
- ⛔ **`EXPO_PUBLIC_*` changes do NOT bust Metro's transform cache** — measured. Without `--clear` the embed can be built from the app build's transforms. This is why the embed gate carries `--clear`.
- ⛔ **A cache HIT makes the probe unrunnable** — no `ios/`, so no `.xctestrun`. Fast-lane-plus-cache is a contradiction until item 4 above lands.
- ⚠️ **`continue-on-error` masks a failed step as `success` in the REST API.** Only the workflow-expression `outcome` tells the truth, and a watcher polling the API will report a green step that exited 65.
- ⚠️ **`findings=0` from the a11y audit is not yet a clean bill** — the anchor wait shipped but has never run. Read `PROBE a11yAnchor … rendered=true` before trusting a zero.

### ⚡ The day's shape, worth naming once

**Three times a confident conclusion preceded a measurement that was already available**: `eraseText`'s
"bounded count" (refuted by the fix changing nothing), "the gate is vacuous" (refuted by grepping the
artifact), and a hermesc log pasted from an earlier run and reasoned on after it had been withdrawn.
Each was caught, none shipped, and each cost real time.

⭐ **The two things that caught them are the same two every time: plant a defect before trusting a guard,
and read the ARTIFACT rather than reasoning about the test.** The pre-flight went 16 → 37 checks this
...
## 2026-08-17 — 4.1.11: the exit, and two decisions that close Phase 4

### 🎯 THE ANSWER TO THE QUESTION 4.1 WAS BUILT TO ANSWER

🎯 Jason, 2026-08-14: *"the point of 4.1 is to see how much of the 3.5 checklist I currently have for the
device build will be covered."* Derived by `npm run audit:coverage`, not asserted anywhere:

| | rows |
|---|---:|
| ✅ **Proven** | **26** — 20 by a native run · **5 by a push-gate spec** · 1 human |
| ◐ Half proven | 19 |
| ▶ Coverable, not built | 60 — Phase 6, human-ticked, **non-gating** |
| 🎯 Permanently device-owed | 26 |
| 🎯 **THE DEVICE PASS** | **52** of 131 real checks |

⚡ **The headline went DOWN twice getting here, and that is the whole value of the instrument.** 4.1.9c
stopped counting declarations as passes (33 → 24). 4.1.10 stopped counting a `PARTIAL:` claim as a whole
row (24 → 21). It went up once, to 26, when the Playwright suites became visible to it at all. **Nothing
was ever marked covered to make a number look better**, and there are now three separate mechanisms that
can take a row back out: a red flow revokes its stamp, a removed spec claim revokes its gate mark, and
`lint:coverage` rejects a mark no harness can account for.

### ⛔ THE NIGHTLY SURFACED A SHIPPED DEFECT BEFORE IT EVER RAN ONCE

The rot guard was reverted in August for its cadence, not its intent, and the plan named a nightly as the
right shape. Adding it turned up something the reverted guard would also have hidden:

**`inputs.*` are empty on every trigger except `workflow_dispatch`.** The iPad tier was gated on
`inputs.device == 'ipad' || inputs.device == 'both'`, so on a **release tag** — the one trigger whose
entire job is a gated native smoke before shipping — it matched neither and **the iPad tier has never run
on a tag.**

⚡ **The iPhone tier survived because it reads the NEGATIVE** (`inputs.device != 'ipad'`), which an empty
string satisfies. So the tag runs came back green, on one tier, looking complete. That is this lane's
signature failure — *a step that quietly did not run is indistinguishable from a step that found nothing*
— one level up, at the job.

⚠️ **And the check written for it reddened correct code within ten minutes.** The first version
pattern-matched `inputs.X ==` fragments and flagged the iPhone job, whose two bare comparisons are
harmless because they are ORed with the safe `!=`. 4.1.9b.8 had logged this exact shape — *"a verifier
whose SCOPE is wider than the rule it enforces"* — and the remedy was the same: **read the structure, not
the text.** `lint:lane` now models GitHub's expression semantics (`||` and `&&` return a VALUE, which is
what makes `inputs.device || 'both'` work at all) and evaluates each job's condition with every input
absent. ⛔ Anything it cannot parse yields `undefined` and the check is **skipped with a stated reason** —
a checker that silently passed on syntax it did not understand would be the blind gate again. Proven on a
planted revert.

### ⭐ THE a11y FINDING IS NOW LOCATABLE

Run `32042253465` gave the audit its first trustworthy reading (`rendered=true` on both tiers) and it was
**not** a clean bill: `hitRegion` = **2 findings, on both tiers** — two real hit targets below the
minimum. They were unlocatable, because the closure incremented a counter and threw the issue away. It now
prints each finding's `compactDescription`.

⛔ **Deliberately `compactDescription` and nothing else.** `XCUIAccessibilityAuditIssue` also exposes
`element` and `detailedDescription`, and Swift **cannot be compile-checked off a Mac** — the probe's own
header says every extra case is a wrong name costing a whole cycle. One property, certain of it, enough to
turn *"2 findings somewhere"* into two addresses. Widen it once a green run proves the line compiles.

### 🎯 TWO DECISIONS

**[D35] 3.5's device pass folds into Phase 6's.** They overlapped the moment the 74 coverable-not-built
rows went to Phase 6 — those rows span §11/§12, which is exactly what 3.5's pass covered. One sitting, one
checklist, no row run twice. ⚡ **3.5 now closes on its BUILD being done**, which it has been for a while;
what was holding it open was a pass that had quietly become a subset of a later one.

**The CodeMagic build is RELEASED.** 🎯 gated it on *"a definitive checklist that covers everything Maestro
and Appium can verify"*. That checklist exists, it is regenerated by a command, and its number is derived
from run results rather than from anyone's memory. The residual ledger — the driver stall, the boot poll
that does not fire, the probe's 1 min → 11 min, the `tutorial-invite` intermittent — rides into Phase 6 as
**known issues, not blockers**: none of them moves the coverage number, and 4.1 was never about them.

---

## 2026-08-17 — 3.5.7.8: the base path, and two things that only an artifact could have told me

### ⛔ DEPLOYING AS-IS WOULD HAVE SHIPPED A BLANK PAGE, WITH A 200

`expo export --platform web` emits asset paths absolute from ROOT (`src="/_expo/static/js/web/…"`), and a
GitHub Pages **project** site serves under `/<repo>/`. Every one of them would have 404'd. That is this
repo's nastiest regression class — the one `route-smoke.spec.ts` exists for — arriving by *deployment*
rather than by code, and the document itself would still have returned 200.

The fix is `experiments.baseUrl`, introduced by an `app.config.js` **overlay** so `app.json` stays the
config and every native build, `prebuild` and the app's own web export are byte-identical when the
variable is unset.

### ⛔ AND THE MECHANISM I WROTE DOWN FOR IT WAS WRONG

The first version used `EXPO_BASE_URL`, and `canvaskit.wasm` DID NOT MOVE. The HTML was rewritten
perfectly; the wasm still 404'd at root. **Metro inlines only `EXPO_PUBLIC_*` into the client bundle** —
`app.config.js` runs in Node and saw the variable, the browser saw `undefined`.

⚡ Renamed to **`EXPO_PUBLIC_BASE_URL`: one variable, two consumers.** The prefix is not decoration; it is
the thing that lets a single name serve the export-time config and the runtime bundle, instead of two
names that disagree the first time someone sets one of them.

⚠️ *A stated mechanism is a hypothesis until an artifact agrees with it* — Law IV, and this is its
cleanest instance yet: I wrote the mechanism INTO A COMMENT while it was false.

### ⛔⛔ THE CLASS WAS SIX SITES, AND MY OWN GREP HID THREE OF THEM

The before-scan reported `locateFile` hardcoded to root in **three** files. It is in **six**. The
difference is a `| head -5` on the search that found them — the other three scrolled off, and a truncated
search reads exactly like a complete one.

Fixing three produced an embed where **Money and Progress drew and Today did not**: `CushionBarCanvas`
sat in the half nobody looked at. ⚡ **What caught it was a probe, not a re-read.** Logging the URL and
failed responses across the arc printed:

```
t=4s  url=…/debt-app-v1/money      dock=1
      HTTP 404 http://localhost:4320/canvaskit.wasm
t=6s  url=…/debt-app-v1/           dock=0
```

— the wasm 404 crashing the Today screen, the demo session dying with it, and the beat-3 assertion failing
as *"element not found"*, which reads like a timing flake and is nothing of the kind.

⭐ **It is now a `no-restricted-syntax` rule**, exempting only `utils/canvaskit.ts`, and proven on a
planted seventh copy. The precedent is the a11y rule directly above it in the same config, whose comment
already said why: *"a rule, not a convention, because the convention failed: this class was fixed by
enumeration and the enumeration was short. The linter knows every site; a person does not."* It was short
again.

### ⭐ THE GATE NOW TESTS THE CONFIGURATION THAT SHIPS

Until this, the embed gate proved the embed worked at ROOT while Pages serves it from a subpath — the one
arrangement nothing exercised. `feedback_check_the_shipped_artifact` exactly. Now the gate builds into
`dist-embed/debt-app-v1` and serves the parent, so the app answers on `/debt-app-v1/` as Pages will.

⚠️ **`serve -s` was dropped, and dropping it IS an assertion.** SPA rewriting answers a missing
`/debt-app-v1/_expo/…js` with `index.html` and a **200**, which would make the local gate strictly more
forgiving than production; Pages does no rewriting. The embed never needed it either — it loads the base
URL once and its router moves by `history`.

⚠️ **`goto('/')` resolves ABOVE a baseURL subpath.** `new URL('/', 'http://…/debt-app-v1/')` is
`http://…/`, so every spec uses `goto('./')` — base-agnostic, and it repeats the path segment nowhere a
rename would have to find it.

⭐ **A new spec fails on ANY response ≥ 400**, and it exists because **no other spec here could see one**:
`zero-egress` watches for FOREIGN hosts, so a same-origin 404 is invisible to it, and the boot specs
assert content, so they go red with a timeout that reads *"the demo is slow"* rather than *"the base path
is wrong"*. It also waits for beat 2 deliberately — CanvasKit's wasm loads **lazily, on a chart's first
mount**, so booting alone never requests the file the whole item is about.

### ▶ .5 IS BLOCKED, AND THE BLOCK IS A RULE WORKING

`workflow_dispatch` only becomes triggerable once the workflow exists on the **default branch** — which is
`release/v1`, and `feedback_never_push_to_release_branch` says to surface that rather than route around
it. `GET /repos/…/pages` also returns **404**: Pages has never been enabled.

Both are 🎯's: land `embed-pages.yml` on `release/v1` (`native-e2e.yml` lives there for exactly this
reason) and set Pages' source to **GitHub Actions**. ⛔ The available workaround is a `push:` trigger, and
it is the wrong one — it would publish states no gate had passed, on the one surface carrying a public
privacy claim.
