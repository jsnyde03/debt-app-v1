...
**▶ ⭐ [AUDIT GATE] GUARDIAN CONVERGENCE AUDIT** — now the active item (the Guardian is functionally complete through 2.6). The flagship adversarial method: rotated multi-lens review → verify each finding → consensus verdict on whether the Guardian is **premium & worth the price**, with a triaged fix list. Phase-3 delight is explicitly NOT a gap here. _Jason will want to engage this gate — kick off deliberately._
- [ ] **A.1** Scope + assemble the input ledger — surfaces: Today Guardian card · **Recovery** · reserve lifecycle (release/attest/walk-back) · Cash Runway · scorecard · notification · trials · variable buffer · free/premium line across all. Pull the flagged after-scan findings (ledger below) as seed input.
- [ ] **A.2** Run the rotated lenses (fan-out, each finding verified): correctness · honesty · premium-value · UX coherence · completeness · free/premium line · substrate integrity · cold-start · **WORDING/VOICE (Jason: every string professional + confident, zero sloppiness — no awkward phrasing/filler/amateur tone; premium copy throughout).**
- [ ] **A.3** Consensus round → the verdict + triage every finding: **must-fix-before-ship · Phase-3 · future.**
- [ ] **A.4** Fold must-fix into the plan (atomic); rest → backlog/Phase-3. 
- **Exit:** a consensus thumbs-up (premium & worth the price) + a triaged fix list; must-fix folded.
- **📋 Flagged after-scan ledger (seed input; detail → LOG):**
  - **2.4:** (a) premium debt-free date can read LATER than free during cold-start (safety-net hold vs free deploying all); (b) multiple honest-but-different numbers on Today (hero Free vs Guardian Cushion vs runway net); (c) Guardian-card + ack-card affordance density; (d) 4b "covered" can overstate · 4c variable-income affordance over-promises · walk-back fires on any surprise · starter-EF deploy overstates cushion · `selectTightTopUp` offers the EF.
  - **2.5:** (2.5.3b wording) the variable-bill buffer surfaces in the `discovery_holdback` ("Settling-in reserve") bucket — mislabels a permanent variance buffer once discovery decays; (e) the buffer shapes cycle 0 only — the forecast/lookahead doesn't flag a variable-heavy FUTURE cycle tighter; (f) the trial card adds to the Today ack-card stack (density); (g) trial-card "Not now" centered/loose (UX polish).
  - **2.6:** (a) **surface unification is partial** — the plan-hero "Overdue payments need attention" line + the Required Actions card still show independently of the recovery card on a shortfall Today (the consensus's "funnel into ONE entry" only partly delivered — UX coherence); (b) free shortfall invite copy ("Premium keeps your cushion at your line") reads off-context when short (wording); (c) shortfall-Today density (recovery card + Required Actions + hero); (d) the "Keep essential" override is one-directional (→ possible ExpenseSheet `deferability` control, backlog).
