...
- **3.5.5 — feature-discovery coach-marks (C).** Calm · one-at-a-time · dismissible · replayable · iOS-16-safe · outside gesture handlers · platform-gated. Scope decision (Jason 2026-07-31): the tutorial stays Guardian-only, coach-marks teach the rest. **Switch-in before-scan already done + inventory corrected** (What-If exists; L5's below-the-fold schedule row is fixed first; the "income-varies toggle" does not resolve) — six sub-steps + the +3 inventory additions are parked in **log 3.5.5 (parked decomposition)**, retrieved at re-switch-in.
...
### 3.5.8 — the App-Preview capture pipeline ▶ ACTIVE

One command on the free GH-macOS lane produces the submittable file, with no Mac in Jason's hands. **[D20]:** Maestro drives · `xcrun simctl io booted recordVideo --codec h264 --mask ignored` records · ffmpeg conforms. Dark mode ([[feedback_dark_mode_screenshots]]); the same timed run already serving the in-app demo and the 3.5.7 embed.

**Apple's rules — RE-VERIFIED 2026-08-06 against ASC Help, superseding the 2026-07-30 note:** 15–30s · **max** 30fps · ≤500MB · H.264 10–12 Mbps or ProRes 422 HQ · audio optional (stereo AAC if present) · in-app footage only, no fingers/device frames · IAP-or-subscription features must be disclosed · ≤3 previews per language. ⚠️ **iPhone takes ONE fixed 886×1920 file for every modern slot (6.9″/6.5″/6.3″/6.1″)** — not a simulator resolution, so the ffmpeg conform is load-bearing, and there is nothing to re-shoot per size.

| # | Step |
|---|---|
| 3.5.8.1 | ✅ **DONE 2026-08-06** — persona → 3 debts/$19,440; all bands still solve, debt-free moves only Sept→Nov 2029, one shared set kept. **Rent DROPPED from the bill mix**: at `clear` the engine allows bills ≈12% of income, so no line could ever look like market rent (swept both levers to prove it). Surfaced + accepted: the debt-free date is a minimums-only projection because the starter EF takes the day-one surplus. Gate green, e2e 129/129. Log: 3.5.8.1 |
| 3.5.8.2 | ✅ **DONE 2026-08-06** — `DemoCaption`: *"Debt-free, one paycheck at a time."* / *"Cushion planning and Recovery require Premium."* Deliberately does NOT read `chrome` — the capture strips the dock and is the run that OWES the disclosure; e2e asserts that asymmetry, verified red. Both themes looked at, at phone size. e2e 130/130. Log: 3.5.8.2 |
| 3.5.8.3 | ⏳ **FIXED, AWAITING CYCLE 3.** Two cycles proved `simctl openurl` cannot start the capture: iOS confirms a custom-scheme open **even with the app frontmost** (cycle 2's `after-openurl.png` disproves cycle 1's fix). ✅ Replaced by a build-time `EXPO_PUBLIC_CAPTURE_DEMO=1` flag + `CaptureAutoStart` — a launch raises no dialog. The distinct-frame guard caught cycle 2 correctly. Log: 3.5.8.3 |
| 3.5.8.4a | ✅ **DONE 2026-08-06** — the date shift **does not reproduce**: November 2029 at both beats, measured on screen in the real run. `balanceAsOfDate` was the right mechanism but lands on the same month. Guarded by an e2e that walks the script and reads what the viewer sees (the headless version compared raw stores no screen uses). Log: 3.5.8.4a |
| 3.5.8.4b | **The unpainted Skia canvases** — first-frame timing; the native path painted fine under Maestro, so this needs the RUNNER, not the web export. Rides 3.5.8.6/.7 |
| 3.5.8.5 | ✅ **DONE 2026-08-07** — `scripts/conform-app-preview.sh`: scale-increase + centre-crop to 886×1920, CFR 30, H.264 high@4.0, and **fails loud on anything ASC would reject**. Dry-run against stubbed ffprobe across 3 cases. Log: 3.5.8.5/.6 |
| 3.5.8.6 | ✅ **AUTHORED 2026-08-07** — `.github/workflows/app-preview.yml`. ⚠️ **No driver: this Maestro build cannot wait out a timed run** (`extendedWaitUntilVisible` absent, `timeout` invalid), and the payoff invitation lands at t=20s → **cycle 1 captures without the tap**. Dry-run caught a silent off-by-three from the recorder pre-roll. **⏳ Needs a runner cycle to prove.** Log: 3.5.8.5/.6 |
| 3.5.8.6b | 🎯 **[DECISION] the payoff-confirmation tap** — the celebration needs *something* to press Confirm. `demoRun` reserves that for the capture driver on honesty grounds, so auto-confirming from the script is Jason's call, not a CI detail. Decide from the first cut |
| 3.5.8.7 | ⏳ **web half DONE 2026-08-07** — `npm run shots:demo` reference set (both themes, filenames mirror the CI artifact). Cents **do** reach the opening frame → **C1's root fixed** (`formatCurrency` padded whole amounts to `.00`); first-mount measured at ≈1s per navigating beat. Native half (first frames from the recording, Skia painted) still owed. Log: 3.5.8.7 |
| 3.5.8.8 | **Review the cut → [DECISION] draft-vs-submitted** — sim capture ships as the asset, or a real-device re-run does. Apple's own wording is "footage captured on device"; decide on the evidence |

**Exit:** a CI artifact that passes ASC's resolution/duration/codec validation, whose every beat's first frame is painted and correct, reviewed by Jason as premium.

> **⚠️ The honest risk (unchanged):** the simulator may not render Skia, `expo-blur` UIKit material, or the finale mesh-gradient faithfully, and recording can drop frames — this is a PREMIUM marketing asset. Auto-capture is the **draft/iteration** path (free retakes, deterministic re-shoots after any UI change); a real-device re-run of the same script is the fallback for the submitted asset. 3.5.8.8 decides on evidence.
> **Why it gets cheap:** the 3.5.0 substrate already supplies determinism (frozen base date → a retake after a UI tweak shows the SAME numbers), named states on demand, a scripted driver, isolation from real data, and a persona that is nobody's actual money. **One script, three uses — in-app demo · marketing embed (3.5.7) · App Preview.**
> **Audit-gate questions — closed at switch-in (detail → log 3.5.8):** Q1/Q2 answered by 3.5.6b · **Q3** the seam already exists · **Q5** the "~5s hook" contradiction dissolved at [D19] (the arc is ≈25s by construction) · **Q6** ffmpeg needs installing, everything else stands · **Q7** the tooling survey → [D20].
...
- **⚠️ A9 · `incomeVaries` HAS NO USER-FACING CONTROL — the whole variable-income feature set is unreachable** *(found 2026-08-07 by 3.5.5's coach-mark inventory scan)*. The flag defaults `false`, is **read by 6 engine modules and written by no `.tsx` anywhere**; the only `SwitchRow` in `PaycheckSheet` is "This paycheck didn't arrive", and onboarding's `PaycheckStep` has none. So **no user can ever turn it on**, which makes VIS-5's debt-free band, `incomeLearning`, §2.0.a lean verification and the variable-income cold-start holdback dead for every real user. It works — `vis5-cone.spec.ts` proves it by seeding `incomeVaries: true` **straight into the store**, never through UI, which is exactly why the gap survived a green suite. 🎯 **Likely Category-1 (version-blocking): shipped, tested, paid-for premium substance that cannot be switched on.** Needs Jason's admission call + a design call on where the control lives (onboarding vs `PaycheckSheet` vs both).
