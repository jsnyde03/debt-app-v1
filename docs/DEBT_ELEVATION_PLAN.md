...
- [ ] **MF.1 Recovery "safe to defer" trustworthiness** [audit #1 BLOCKER · #3 · #8 major] — exclude `isAutopay` from deferrable (+ honest "reschedules your plan, not the biller's charge") · treat uncategorized/`other` as **essential** (never pre-suggest a bill we can't classify) · add the empty-deferrables copy branch · **+ Tier-1:** `keepEssential` no longer resets read-freshness (skip `stampInputsFresh` for a deferability-only change).
...
- [ ] **MF.3 Free honest shortfall read** [audit #4 major, triple-confirmed] — free told "won't cover everything / about $X short"; only the built plan stays premium · **+ Tier-1:** state-aware invite (pitch the recovery value in a shortfall, not "cushion at your line") · "protected from today" → "protected, **starting today**" (audit #9).
...
- [ ] **MF.7 Tier-2 polish pass** (Jason ✓ fix-now) — `cycle`→`paycheck` terminology sweep · variable-bill buffer label (own branch, not "Settling-in reserve") · trial-card "Not now" layout tidy.
- [ ] **⭐ MF.8 [AUDIT GATE] Round-2 convergence audit** (Jason: don't leave Guardian until CONSENSUS it's premium-level) — same 4 adversarial lenses + criteria as round 1, re-run against the fixed code; iterate any surviving must-fixes until **you + the agents reach consensus the Guardian is premium & worth the price.** THE gate to exit Guardian.
- **Exit:** MF.1–MF.7 fixed + verified (both themes · tests · e2e) → **MF.8 round-2 audit reaches consensus premium-level** → 2.7 BNPL. **Tier-3 → Phase 3** (hero "Free"→"Safe" label · ack-card density coordinator · "Keep essential" toggle+undo). **Refuted (no action):** 2.4a-positioning · 2.4b-contradiction · 2.4c-attestation · 2.4d-starter-EF · substrate SAFE · recovery math SOUND.
