...
- **▶ 3.5.8.9 built 2026-08-07** — the app now marks its own paint moment and the pipeline FINDS the cut; `LAUNCH_ALLOWANCE` deleted. Gate green, proved locally against a flagged web export. **Remaining: one cycle (3.5.8.9.5).**
...
| 3.5.8.9.5 | ▶ **One cycle, then verify from the artifact** that every beat's FIRST frame is painted — the check cycle 8 failed. The 2fps sheet also measures the post-navigation paint lag, which is what 3.5.8.8 needs |
