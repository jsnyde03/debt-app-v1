...
| ▶ **.5.8** | **`[D79]` RE-AUDIT of class 5 — fresh agents, cumulative, never the fixer.** **8.1** brief + pin: the class-5 fix range, the reading rules, and the cumulative questions *(interaction with closed classes · guards whose pinned files moved · closure by planting)* · **8.2** dispatch fresh lane agents, no sub-agents, writing findings as they go *(🎯 2026-09-14: from a fresh session, off `CLASS5-START-HERE.md`)* · **8.3** record findings split by origin — attributable to class 5's fixes vs reservoir · **8.4** fix the attributable ones and file the reservoir onward · **8.5** repeat 8.2–8.4 until a round finds zero defects attributable to class 5's fixes | 0 new defects attributable to class 5's fixes |
