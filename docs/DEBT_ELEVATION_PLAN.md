...
| 3.5.8.3 | ⚠️ **CYCLE 1 FAILED IT (2026-08-07)** — `openurl` from SpringBoard raised iOS's *"Open in 'Debt Planner (RN)'?"* prompt, nothing dismissed it, and **30s of Home Screen was recorded, conformed and uploaded GREEN**. Cycle 2: leave the app running (no app-switch to confirm) + a distinct-frame guard so a static capture fails. Log: 3.5.8 cycle 1 |
