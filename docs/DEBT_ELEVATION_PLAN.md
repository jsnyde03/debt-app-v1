...
- **✅ 3.5.8.10 CLOSED 2026-08-08 (cycle 12)** — 🎯 *"some time before it starts"* was **3.6s of black then the capture slate** in the shipped mp4: `-ss` before `-i` is a keyframe seek on a VFR `.mov`, the same lesson cycle 7 learned and never carried to the deliverable. ⚠️ **The real miss: every check looked at PNGs from `raw.mov`; nothing ever looked at a pixel of the file being shipped.** Now frame zero is a painted Money screen, verified by decoding the mp4 — and the conform asserts no surviving slate + no black open, with a stubbed test (5/5) because that guard had already broken twice.
