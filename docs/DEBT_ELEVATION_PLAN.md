...
**▶ 2.11 ⭐ Revenue spine (the active build)** — the premium FEATURES are done (Guardian · BNPL · scan · affordability), so the monetization plumbing is the highest-value next: it turns substance into revenue. RevenueCat + paywall (port Gig — no IAP wiring exists in the RN app yet) + Lifetime 2nd offer + portfolio-sub seam + analytics + Sentry. _**Switch-in = the [DECISION] pricing gate — decide-first, before ANY StoreKit SKU (a collaborative call w/ Jason).**_
- [ ] **2.11.1 [DECISION GATE]** guarantee window/terms (honest "not charged until day 30" fallback if StoreKit can't refund) · Lifetime scope · pin annual + Lifetime prices · confirm the 4-tier ladder holds · **the paywall/moat privacy CLAIM = honest "100% on-device, nothing leaves your phone" (NOT "E2EE iCloud backup" — that ships in Phase 5)** → decompose 2.11.2+ (RevenueCat wiring · paywall port · portfolio-sub seam · analytics + Sentry). "Watches every paycheck" copy gated on 2.4.10. **Launch-FLIP stays Jason's value gate** (value is now largely shipped).
- **Exit:** the revenue spine wired + a paywall shipped (launch-flip deferred to Jason); verified both themes + tests. **This closes Phase 2 → the Premium-framework audit gate below.**
