...
- **Active item:** **▶ 2.7 BNPL as a first-class obligation** — 2.7.1 gate ✅ · 2.7.2 installment model ✅ · **2.7.3 native capture/display ✅ (2026-07-24, e2e 19/19, both themes)**. **▶ NEXT = 2.7.4** (Guardian-aware biweekly cadence — the differentiated core; switch-in decomposes it + folds the 2.7.x after-scan). **✅✅ GUARDIAN COMPLETE + AUDIT PASSED. ✅ 2.6 · 2.5 · 2.4 COMPLETE**.
...
- [ ] **2.7.4** Guardian-aware cadence — the differentiated core: a biweekly installment lands *between* a monthly earner's paychecks; surface it as a lumpy near-term obligation in crunch detection (`biweekly` recurrence already exists). **+ 2.7.2/2.7.3 after-scan: snowball extra targeting an interest-free BNPL (whole installments vs arbitrary $) · one-time BNPL w/ >1 payments dropped from rollover · cadence-aware amortization (`AmortizationSheet` is monthly-only, misstates a biweekly BNPL's schedule).**
