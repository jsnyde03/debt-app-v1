...
| ✅ | **.7.3 — the one WRONG-REMEDY** | DONE 2026-08-24. **V2-6** closed by 🎯's pick (scroll into room) — but the subject was **362 pt**, not the ~255 I estimated, so the reveal needed 263 px against a `maxScroll` of 196 and was impossible until `TutorialTarget` moved onto the **scrub view** inside `TrajectoryChart`. ⛔ Found on the way: `measure()` is in WINDOW coords, so **a scroll left the callout behind while its subject moved** — a defect no lens filed. ⚠️ I broke f's self-occlusion pin; its assertion encoded *placement* (`bottom <= top`) as a proxy for its own stated property — rewritten to zero-overlap, **strictly stronger**, and plant-verified. Detail → log |
