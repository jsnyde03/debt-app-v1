...
- [ ] **2.9.6 Save-for-it (short) — DEFECT-DRIVEN REDESIGN (Jason 2026-07-25).** _Defect: a savings goal is `optional_goal` → funded AFTER debt in `allocatePaycheck`, so the "ready by {date}" promise is false for a debt-carrying user._ Fix + expand:
  - [ ] **2.9.6a [engine]** a `priority?: boolean` on `Goal` → `allocatePaycheck` funds priority goals BEFORE the snowball (capped at target, like the starter-EF). Reconciliation-tested.
  - [ ] **2.9.6b [selector]** compute the honest debt-free-date cost (rounded/qualitative) of prioritizing; build the 3 save options.
  - [ ] **2.9.6c [UI] a focused Save-for-it SHEET** (hybrid: inline apply, sheet for the deliberate save) — **3 options each w/ its trade-off**: Save-fast (prioritize, ~cost later), Balanced (gentler prioritized pace), Debt-first (normal post-debt goal, no cost, no firm date) → **user picks + SIGN-OFF confirm** → creates the goal with the chosen priority. Never a false date.
- [ ] **2.9.7 verify** — tests + e2e + both themes.
