...
- **3.5.4.11 — reimagine the demo for the App Preview** *(supersedes .6's Guardian-only script and .7's in-app entries)*: un-ship the entries (`isDemoReachable` → dev/QA), let the scripted run **navigate between tabs**, and carry the arc past the Guardian to the debt-free date and the payoff celebration. Storyboard first, then build.
