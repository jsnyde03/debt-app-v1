...
| **S1.11.6.1** | **`D4-11` — the route's own checks.** Three of five set-identity assertions are unreachable *(a `die()` that always fires first; `Map` keys that are unique by construction)* and two of the three numbers in the success line are constants | every printed number is measured, and each `die()` is reachable under a plant |
| **S1.11.6.2** | ⛔ **THE CONSUMERS HALF.** The route emits files *changed* since the pin, so a two-producer disagreement is **half-routed by construction** — the fix touches one producer and the route sees one producer. `A-F4` is the proof: `buildPayoffTrajectory.ts` routed to **nobody** | the route emits the CONSUMERS of every changed producer |
| **S1.11.6.3** | **`D4-7` — S0 is never routed.** The route emits `first-look` for S1 and never for S0, so the **62 never-swept S0 files** ([D76]) are in no lane of any round | S0's unswept set appears in a lane |
| **S1.11.6.4** | **THE MEMORY PROTOCOL, PERMANENT.** Heap capped at 1536 MB · ⛔ an OOM is a **finding**, never a retry · no whole-monorepo typecheck · kill servers you start · verify every restore | the protocol is in the script, not in a doc a future session may not read |
