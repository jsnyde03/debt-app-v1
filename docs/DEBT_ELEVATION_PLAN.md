...
- [ ] **2.5.1** Schema — trial/intro fields on `RequiredExpense` (isTrial · full-charge price · first-full-charge date), optional/backfill-safe (no version bump; parses like `cycleTopUp`). **✅ built** (not yet verified/committed).
...
- [ ] **2.5.3** Core (pure, reconciliation-tested) — (a) trial→full-price projection: future cycles use `fullAmount` on/after `fullChargeDate`; (b) variable-bill reserve — Guardian plans a **modest bounded % buffer (~10–15%) on `expenseType:'variable'` bills, folded into the cushion (NOT stacked on the §2.0 discovery holdback)** (Jason ✓ 2026-07-24). _[BNPL-expiry cut — already handled.]_
