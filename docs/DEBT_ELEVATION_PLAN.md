...
| # | cluster | contents |
|---|---|---|
| **P6.8.7a** | ⭐ **GATES FIRST** | ✅ **(i) DONE 2026-08-21** — `aria-allowed-attr` wired into `a11y-axe.spec.ts`, `a11ySelected` **retired**, and all 6 sites re-roled to `a11yChecked` *(3 `radio`, 2 → `checkbox`, 1 → `radio`)*. ⚡ **RN has no `aria-pressed`** — `tsc` refused the docstring's own suggested remedy, so the button sites were re-ROLED. 10/10 axe tests green. Detail → log. **Remaining:** `lint:apostrophes` blind to `&apos;` **(W1-1)** · `gateSources` `EXTRA_FILES` misses 4 build-affecting files **(W1-4)** and wrongly excludes `.github/**` **(W1-12)** · `check-native-a11y-props` closed at 4 of 6 **(W1-10)** · `web-e2e.yml` omits `test:stamp` + `test:e2e:embed` **(W1-3)** |
| **P6.8.7b** | **COPY — no engine risk** | **A4** `WelcomeStep.tsx:19` promises a premium feature *(must beat the P6.19 freeze)* · **C6** trust line at first data entry *(wording already written — `DEBT_BENCH_TRUST_FIRSTRUN` §R1, T1+T3)* · **M1-8** hide the inert analytics row · **L1-22** the 72-string apostrophe sweep *(7 assertions + `duplicate-copy-baseline.json` must move in the SAME commit)* · **A1/A2/A3** drafted into the repo — ⚠️ **A2's live pages are in ANOTHER repo and A3 is an ASC checkbox; neither is fixable from here** |
