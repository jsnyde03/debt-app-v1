...
- **Active item:** **▶ 2.7 BNPL as a first-class obligation** (next; switch-in = before-scan + design alignment w/ Jason, like 2.6). **✅✅ GUARDIAN COMPLETE + AUDIT PASSED — 4/4-lens CONSENSUS it's premium-level & ship-ready** (2 audit rounds; all must-fixes MF.1–MF.8 fixed + verified). **✅ 2.6 · 2.5 · 2.4 COMPLETE**.
...
**▶ 2.7 ⭐ BNPL as a first-class obligation** — the Guardian is done + audit-passed; BNPL is the next build (Jason: pull into Phase 2 after the Guardian audit). BNPL works today (captured · projects via balance-depletion · interest-free · amortization view) — this is *elevation*, not a fix. _**Switch-in = before-scan (verify the BNPL model vs CURRENT code) + design alignment w/ Jason BEFORE building 2.7.2+ (design-shaped, like 2.6.1).**_
- [ ] **2.7.1 [DESIGN GATE]** Switch-in before-scan + design alignment — map the current BNPL model/surfaces vs code (the latent gap: engine uses `balance`+`minimumPayment`, display uses `scheduledPaymentAmount`×`remainingPayments`, nothing reconciles them), then agree the BNPL-native design w/ Jason → decompose 2.7.2+ to leaves.
- [ ] **2.7.2** Coherence — make `scheduledPaymentAmount × remainingPayments` the canonical BNPL model; reconcile/validate `balance`+`minimumPayment` so the forecast + amortization view never disagree.
- [ ] **2.7.3** BNPL-native capture/display — provider (Klarna/Affirm/Afterpay) · installment cadence (often biweekly) · "payment 2 of 4" progress · next-installment date — out of the generic debt sheet.
- [ ] **2.7.4** Guardian — surface the next BNPL installment as a near-term lumpy obligation in crunch detection (biweekly BNPL lands between paychecks).
- [ ] **2.7.5** Premium angle — "Can I afford this BNPL?" (shares the inverse-Guardian engine w/ future-feature ①) + a consolidated BNPL calendar — a differentiated, under-served acquisition lever.
- **Exit:** BNPL is a coherent first-class obligation (engine + display reconciled · native capture · Guardian-aware · the premium angle); verified both themes + tests + e2e.
