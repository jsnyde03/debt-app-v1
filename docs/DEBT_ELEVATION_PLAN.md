...
- **⛔ [4.1.3] THE `debt-row-actions` COACH MARK NEVER APPEARS.** ⚡ **Evidenced, and it is a PRODUCT defect, not a test one:** flow 01's `smoke-seeded-portfolio` screenshot is a **fresh install** (`clearState: true`) sitting on Money with three debts — persisted record definitionally empty, session set empty, no suppressors, `Platform.OS === 'ios'`, `view.order.length > 0` — and **no mark**. So `show()`'s four guards are all clean and it still does not render. ⚠️ **Likely, NOT concluded:** `useCoachMark` returns early while `ready` is false and only subscribes when it flips true, but the store hydrates async — so the row's single `onLayout` → `invalidate` has already fired by then, and the listener set has no replay. **Four earlier explanations for flow 08 were all about the reset path, which was never involved** — this one needs its own probe rather than another guess. **Blocks 08's §13.3/§13.4 assertions.**
