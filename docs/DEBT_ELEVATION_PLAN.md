...
- **Active item:** **▶ 2.7 BNPL as a first-class obligation** — 2.7.1 design gate ✅ · **2.7.2 installment-native model ✅ (2026-07-24, gate green)**. **▶ NEXT = 2.7.3** (BNPL-native capture/display — provider · "2 of 4" · derive/hide balance for BNPL; design-shaped). **✅✅ GUARDIAN COMPLETE + AUDIT PASSED. ✅ 2.6 · 2.5 · 2.4 COMPLETE**.
...
- [ ] **2.7.3** BNPL-native capture/display — provider (Klarna/Affirm/Afterpay/PayPal/Zip) · "payment 2 of 4" progress (derived) · next-installment date — out of the generic debt sheet. **+ from the 2.7.2 after-scan: for BNPL derive/hide the `balance` field (terms win — capture scheduled+remaining, not a free-typed balance) · derive `originalBalance` at capture · upgrade one demo BNPL to installment-native (biweekly) to showcase "2 of 4".** Lean.
- [ ] **2.7.4** Guardian-aware cadence — the differentiated core: a biweekly installment lands *between* a monthly earner's paychecks; surface it as a lumpy near-term obligation in crunch detection (`biweekly` recurrence already exists). **+ decide whether snowball extra should target an interest-free BNPL at all (2.7.2 after-scan) — pay whole installments vs arbitrary dollars.**
