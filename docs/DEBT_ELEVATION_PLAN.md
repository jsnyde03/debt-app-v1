...
- [ ] **2.6.2** Deferability model (schema + heuristic, pure, tested) — optional `deferability?: 'essential'|'deferrable'` override on `RequiredExpense` (backfill-safe, null → derive); pure `classifyDeferability(expense)` = override ?? category-default (minimum debt payments always essential); reconciliation-tested.
- [ ] **2.6.3** Recovery engine (pure, tested) — `buildRecoveryPlan(shortfall, obligations, classify)` → cover-now set · safe-to-defer ranked (deferability then amount, "closes $X of the gap") · running gap math · closeable flag + the honest residual-gap "can't close it this cycle" branch. Reconciliation-tested.
- [ ] **2.6.4** Defer action + surface unification — `deferExpense(id)` advances the due date one cycle; route the plan-hero short/overdue + Guardian shortfall + looming-crunch into the ONE Recovery entry (the card expansion).
- [ ] **2.6.5** Card-expansion UI (inline in the Guardian shortfall card — same voice/visual) + per-bill override flip + one-tap apply + the Interest-Saved counterfactual. Copy = placeholder → convergence-audit polishes.
