...
- **`QA_TOOLS = true` ships in TestFlight and MUST be flipped false before submission** (`git grep QA_TOOLS`). It is what makes the demo reachable at all.
  - ⛔ **[raised 2026-08-17] THE FLIP INVALIDATES ANY LANE COVERAGE THAT RIDES A QA DOOR, at exactly the moment it matters.** Phase 6's device pass runs against the **flipped** build, but the Maestro/XCUITest lane runs against an unflipped one — and 4.1.10's proposed route to the scripted demo is a `qaEnabled()` control in More. So a row can be green in the lane and unreachable in the artifact that ships. ▶ The flip needs an explicit **re-run of the lane after flipping**, or every QA-door-dependent row falls back to human. ⚠️ Not hypothetical: `probeCoachMark`, the `coach-probe` readout, `suppressorReasons`, `RING_AUDIT` and now `rm-probe` are all `qaEnabled()`-gated.
