...
| ▶ **.5.4d** | **`'required-plan'` IS THE WRONG QUESTION FOR MOST OF ITS ASKERS.** Measured on rendered fields: the plan hero *(9 holes, incl. a debt-free date over a lost APR)*, the Guardian card, affordability and windfall routing *(7 each)* state wrong figures over a lost autopay amount or goal field. ⭐ **[DECISION] 🎯 2026-09-13: exact claims per surface** — a new `'paycheck-plan'`, `'required-plan'` narrowed to the obligations, the hero on `'solved-projection'`. Detail → log | every `'required-plan'` asker refuses exactly when its rendered figures move; `C1-1` stays closed |
| ✅ **.5.4d.1** | **SWITCH-IN DONE 2026-09-13** — nine askers by query; fixture caveats settled over four probe rounds; measured on rendered fields. ⚠️ Narrowing `'required-plan'` would blind `cushionLine` *(`C1-1`)* — it moves too. Detail → log | ✅ |
| ✅ **.5.4d.2** | **DONE 2026-09-13** — **0 holes across every asker on 22 shapes**; `cushionLine` 22/22. ⚠️ My narrowed route was short twice *(windfall + bill reserve, then groceries on a short paycheck)*; the paywall lead carries 7 named over-suppressions. Detail → log | ✅ |
| **.5.4d.3** | Claims into `CLAIM_FIELDS`; the completeness ledger re-decided; the per-surface assertion extended to every asker | suite green; plants red for their own reason |
| **.5.4d.4** | Rewire — hero *(figures gated, folds `C1-5`)* · Guardian card · Live Activity · widget line · affordability · windfall · required actions · paywall lead · `cushionLine` | typecheck 0 · `lint:trust-claims` floors declared |
| **.5.4d.5** | e2e — the Guardian card over a lost goal target; the hero's date over a lost APR — planted both ways | collected by name |
| **.5.4d.6** | Close — proofs drained, `lint:rn` alone, push. ▶ **Then: save-for-it**, which asks no trust question at all | CI green |
