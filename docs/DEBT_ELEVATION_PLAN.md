...
| ▶ **.5.4** | ✅ **`C3-9` DONE 2026-09-13** *(planted: the un-fix renders **"November 2026"** where the fix renders `—`, balances readable, APR not)*. The fix is a **split**, not suppression — `pct` stays on `'debt-balances'`, `heroDate` + the view gag + what-if take the predicate, and `journeyLine` takes **neither**: its arms sit on opposite sides, so `selectJourneyTotals` now returns `lineIsProjected` from the same expression that picks the arm. ⚠️ First fixture on this screen to poison an **APR** — every other poisons `balance`, so the gag fired correctly by accident and nothing covered `C3-9`. ✅ **`C3-8` DONE 2026-09-13** *(planted: removing the guard renders **`$11,800`** where the fix renders "Some figures unread")*. ⭐ **The received figure is BELOW the anchor** — the repaired APR is `0`, so the projection accrues no interest and **understates** the debt; the finding reports the overstatement direction, so **the sign is a property of the fixture, not the defect**. ⚡ **Second independent instance of one shape, one screen apart** — a guard correct about the claim it asks, blind to the claim the number is computed from, the gap opened by a previous fix's deliberate narrowing. **That is the case for the inherited iterating assertion.** ▶ **Next: `.5.4a`** — the predicate both of these adopted OVER-SUPPRESSES, measured after they landed; `C3-11` waits on it. — **THE SURFACES, BY THAT PREDICATE** — `C3-13` *(guard at the CALL SITE — the projection is the argument)* · `C3-8` *(the HERO, not the rows)* · `C3-9` · `C3-11` · `C3-5` *(the unfixed `liveActivitySync` twin: stamp-before-verify)* · `C3-1` · `C1-5` · **`C3-2`+`D2-12` as ONE step** *(the overloaded `''`)*. ⚠️ Six were walked in one list last round and **`C4-4` was the measured hole a finding came through** | every surface routed through the predicate, proven by planting at each |
| ▶ **.5.4a** | **THE PREDICATE OVER-SUPPRESSES — split into two CLAIMS.** `'row-figures'` subsumes every claim, so `mayStateProjectedFigure`'s first conjunct never decides *(0 of 102)* and a lost goal target blanked the debt total *(Money: 15 of 19 refusals move nothing)*. ⛔ Narrowing to debt fields is the opposite error — a lost rent moves the date **earlier**. Routes validated at **0 mismatches** before writing. `C3-11` waits on it. Detail → log | each claim refuses **exactly** when its figure moves, per repair variant, both directions |
| ✅ **.5.4a.1** | **SWITCH-IN DONE 2026-09-13** — every carried figure re-measured *(none was on disk)*; ⚠️ my first probe was vacuous *(a zero-length pay window)*. Detail → log | ✅ |
| **.5.4a.2** | `'projected-balance'` + `'solved-projection'` into `CLAIM_FIELDS`; `mayStateProjectedFigure` retired; the iterating assertion over `REPAIRABLE_MONEY_FIELDS` × six shapes | trustSelectors suite green · completeness ledger re-decided |
| **.5.4a.3** | Rewire Money's hero · Progress *(view · what-if · date · the journey's projected arm)* · the widget snapshot | typecheck 0 · `lint:trust-claims` floors re-declared |
| **.5.4a.4** | e2e on the user path — a lost goal target keeps Money's total; a lost `typicalAmount` keeps Progress's date; `C3-8`/`C3-9` stay green | collected by name |
| **.5.4a.5** | Plants — a debt-only solved route · the projected route without `apr` · Money on the wrong claim · a named goal route | each reds for its planted reason |
