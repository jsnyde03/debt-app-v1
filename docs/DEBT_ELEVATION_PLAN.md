...
- [x] **2.9.5 Cover-from-savings (tight) ✅ (2026-07-25)** — `selectAffordability.coverFromSavings` (tight only; a discretionary SAVINGS goal, never the EF) → a `[Cover $X from {goal}] & apply` primary button that applies the purchase + `applyTightTopUp` in one tap (Undo reverses both via a negative top-up); "Apply anyway" demoted to a text button. +5 app asserts; both themes verified; e2e 23/23. **✅✅ 2.9 CAN-I-AFFORD-THIS? FULLY COMPLETE.**
