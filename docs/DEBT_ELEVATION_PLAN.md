...
- **🔵 FIRST THING NEXT SESSION — capture cycle 14** (`app-preview-20260808-14`, run **31244485894**) was in flight at session end. **Download the artifact and look before anything else.** Three things must hold: the log says **"✅ the celebration fired"** and **"✅ the chart reveal ran"**; `beat-6-celebration.png` shows *Store card · Vanquished*; and — the one no guard covers — **decode the mp4's tail** and confirm the celebration has **≥2s** of room, not cycle 13's 0.1s. ⚠️ **Decode with Edge, not ffmpeg** — see the Env line.
- **⏳ 3.5.8 — everything built and approved except that last check.** 🎯 Cycle 12's cut **APPROVED 2026-08-08**; both decisions resolved and shipped (6b auto-confirm · .8 sim-capture-plus-guard). Cycle 13 proved the celebration reaches the file but landed it in the final 0.1s — the script's stage timers drift ~4s on a loaded runner, so the conform's duration became a **29s ceiling**. Cycle 14 is that fix. Detail → log 3.5.8 cycles 8–14.
...
- **⚠️ Env — no ffmpeg here can read H.264.** Playwright's bundled build is `--disable-everything`, so inspecting a capture MP4 locally means Edge: `chromium.launch({ channel: 'msedge', args: ['--allow-file-access-from-files'] })`, `goto` the `file://` URL, seek, draw to canvas, read `getImageData`. Without that flag the canvas is tainted and luma readings are unavailable. It is how cycle 10's black-and-slate opening was found.
