...
| ✅ **.5.4d** | **CLOSED 2026-09-13 — `'required-plan'` was the wrong question for most of its askers.** ⭐ **[DECISION] 🎯 2026-09-13: exact claims per surface** — a new `'paycheck-plan'` *(Guardian card · Live Activity · widget line · affordability · windfall · paywall lead · `cushionLine`)*, `'required-plan'` narrowed to Required actions, the plan hero on `'solved-projection'` with its split gated *(folds `C1-5`)*. 0 holes on 22 shapes; plants Q1–Q6 unit and R1–R4 e2e red for their own reason. ⛔ The rewire made four refusals say *"has to cover"* over goal and APR losses — one corrected lead now. ⚠️ Two render proofs never existed *(affordability, windfall)* → `.5.7`. Detail → log | ✅ |
| ▶ **.5.4e** | **SAVE-FOR-IT — "asks no trust question" is TRUE and NOT A LIVE DEFECT.** Measured: its one door (`openSaveSheet`) renders only in the affordability card's short-verdict branch, after the `'paycheck-plan'` refusal, so it cannot open over an unread input. ⚠️ The mount itself was ungated — a second door would bypass silently. Detail → log | the sheet's mount refuses over a loss however it is opened, pinned and planted |
| ✅ **.5.4e.1** | **SWITCH-IN DONE 2026-09-13** — rules re-read; one door, behind the refusal; the pre-authored premise was true of the source and not of the app. Detail → log | ✅ |
| **.5.4e.2** | Gate the mount on `!unreadPlanInputs`; a `requiredPlanTrust` row pins the conjunct; plant it out | the row reds on the plant; `test:app` green |
| **.5.4e.3** | Close — proofs drained, `lint:rn` alone, push. ▶ **Then: `C3-5`** *(the `liveActivitySync` twin: stamp-before-verify)* | CI green |
