...
**▶ 2.5 Smart obligation quality layer** — descoped to capture fields + an on-device heuristic (Core ML deferred); it flags lapsed-trial / one-off / finite-BNPL obligations at the payday confirm to protect Guardian accuracy. _Structure-first; the switch-in before-scan verifies scope vs current code, and the capture-UI + heuristic design align w/ Jason at build._
- [ ] **2.5.1** Schema — add the capture fields to the obligation model (category · trial/first-seen date · BNPL term / installments-remaining · amount-variance) + migration.
- [ ] **2.5.2** Capture UI — surface the fields in bill/expense entry (design-shaped w/ Jason).
- [ ] **2.5.3** Heuristic core — pure on-device logic: finite-BNPL auto-expiry (via term) · trial-lapse (via first-seen) · amount-variance flag; reconciliation-tested.
- [ ] **2.5.4** Wire into the plan / Guardian — surface the quality signals at the payday confirm (protects Guardian accuracy).
- [ ] **2.5.5** Verify — both themes + tests (a scenario if apt).
- **Exit:** obligations carry quality signals; the heuristic auto-flags finite/lapsed/variable items; Guardian accuracy protected; green.
