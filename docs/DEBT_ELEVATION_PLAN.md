...
- **▶ ACTIVE BUILD: 3.5.6 — verify + close Phase 3.5,** decomposed below. **3.5.6.1 + 3.5.6.2 ✅ DONE 2026-08-10.** **▶ 3.5.6.3 (consolidate the device debt) is now the active sub-step.** **3.5.5 ✅ COMPLETE.** Gate **145/145**.
...
| **3.5.6.3** | **▶ IN PROGRESS — consolidate the 3.5 device-QA debt** into the Phase-6 ledger — the walkthrough's §3.5.3 block, the demo's native lane, 3.5.5's three owed items and 3.5.6.2's four, in one place a device pass can actually be run from |
...
**▶ 3.5.6.3 — decomposed (the ACTIVE sub-step):**

| # | Step |
|---|---|
| 3.5.6.3.1 | **Gather** every 3.5 device-owed item from where it currently hides — the §3.5.3 walkthrough block, 3.5.4's demo native lane, 3.5.5's three, 3.5.6.2's four, L4's unrecoverable round-10 residue — into one list, each with what to DO on the device, not just what is unproven |
| 3.5.6.3.2 | **Fold that list into the Phase-6 device-QA ledger** in this file, deduped against what §3.5.3 already says, so the device pass is run from ONE place |
| 3.5.6.3.3 | **Mark what a Maestro/iPad simulator lane could take off the device's plate** — the ring-origin invariant is the clear candidate (native layout, web-blind, and flow 04 already proves the pattern works) |

**Exit (3.5.6.3):** no 3.5 device-owed item lives only in a comment, a spec docblock or a log entry.
...
- **§3.5.3 the Guardian WALKTHROUGH (added 2026-08-04 by the round-4 device lens).** ⚠️ This whole feature was verified almost entirely by a WEB export; none of the below is observable there, and until now none of it was written down anywhere the device pass would actually be run from.
  ...
  - **Skia cushion-bar paint timing on beat 1** — the first captured frame of the walkthrough showed the bar unpainted (a CanvasKit artifact on web, but beat 1 spotlights that exact card, so a late paint on device is a poor first impression). ⚠️ **Seen twice more on 2026-08-10** (3.5.6.1's spacing frames and one 3.5.6.2 light beat-1), and NOT in the re-shoot — so it is **intermittent**, which is what a paint race looks like, and the device question is correspondingly more live.
  - **⚠️ [3.5.6.2] The iPad ring-origin invariant is held by NOTHING automated.** `TutorialOverlay` subtracts the overlay's origin because the iPad tab-bar rail puts window and local coordinates ~700pt apart. Measured 2026-08-10: **removing that correction changes nothing on RN-web at any width** (the origin is 0 there), so a web assertion for it was written and deleted for claiming a guarantee it could not give. Needs the Maestro/iPad simulator lane, or a real device.
  - **[3.5.6.2] Beat 5's ring on iPad LANDSCAPE** clamps to a short stage, and its bottom border cuts through the card's closing disclaimer line. Portrait is clean. A composition call, on hardware.
  - **[3.5.6.2] The `payoff-schedule` coach-mark's placement is unverifiable on web** — it lands at y≈1266 in an 874pt viewport (RN-web puts the absolutely-positioned layer into document flow inside a Modal). Whether it sits correctly beside its control is a device-only question. Frames: `apps/rn/capture-ref/phase35/<theme>/coach-payoff-schedule.png`.
