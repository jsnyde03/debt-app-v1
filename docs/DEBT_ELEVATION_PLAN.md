...
- [ ] **2.6.5** Card-expansion UI (inline in the Guardian shortfall card — same voice/visual) + per-bill override flip + one-tap apply + the Interest-Saved counterfactual. Copy = placeholder → convergence-audit polishes. _[design-shaped — align the card visual/interaction w/ Jason before building]_
