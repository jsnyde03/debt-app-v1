...
| # | sub-step | exit line |
|---|---|---|
| ▶ **S1.13.7.9.1** | **CLASSIFY THE 51 BY CAUSE.** ⛔ **MEASURED SO FAR: IT IS MULTI-CAUSE — the first single-cause hypothesis was tested on a second cluster and refuted.** ① `expense-reserve` is `S1.13.7.1`'s shared-fixture flip: the default debt/bill moved from **61 days overdue** to `day(6)`/`day(4)`, and flipping just those two back to `day(-6)`/`day(-4)` takes it **10/11 → 11/11**. ② `swipe-mark-paid` (3) + `a11y-axe`'s fence (1) are **unaffected by that** *(4 failed / 9 passed BOTH ways)* and **not mine** *(3 failed with `C1-1`'s seven files reverted)* — cause still unknown, inside the 36. ⚠️ `swipe-mark-expense-e0` and `Mark paid` are simply **absent from the DOM**, and `a11y-axe` counts **0 `[inert]`**, so both are the swipe/fence mechanism rather than an assertion | every failure attributed to a cause, not just to a spec |
