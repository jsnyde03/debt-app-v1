...
| ▶ **.5.4c** | **`C3-13` — A PROJECTED $0 IS CLAIMED AS A COMPLETED PAYOFF.** Measured: premium, $100 owed, verified two months ago → *"You're debt-free · Every balance is cleared"* above the card asking to confirm that payoff. **Three readers, one cause** — plan state, the Guardian's *"To savings"*, the reserve release's *"your savings"* — all ask liveness of a projected store. ⭐ **[DECISION] 🎯 2026-09-13: fix at the PROJECTION**, reversing `.5.1` correction #5. Detail → log | no surface states a completion the confirmed balances have not reached; a confirmed $0 still celebrates |
| ✅ **.5.4c.1** | **SWITCH-IN DONE 2026-09-13** — reproduced exactly, both controls hold; population derived by query. ⚠️ Corrects my own grouping: `C3-13` is a completion claim, the Guardian card's holes are claim routing — two steps. Detail → log | ✅ |
| **.5.4c.2** | `withProjectedBalances` records each debt's confirmed balance, never persisted; the liveness owners read it | every liveness reader answers from the anchors on a projected store; raw stores unchanged |
| **.5.4c.3** | The iterating assertion — every liveness reader × {projected $0 · confirmed $0 · ordinary · free}, plus the payoff invitation surviving | green; the naive over-fix *(allocation on anchors)* planted and caught |
| **.5.4c.4** | e2e on Today — projected $0 keeps the invitation and drops the banner; confirmed $0 keeps the banner | collected by name; both directions planted |
