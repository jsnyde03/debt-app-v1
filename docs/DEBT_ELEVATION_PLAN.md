...
| **4.1.9c** | ⭐ **"Covered" must mean PROVEN, and proof ticks the checklist** *(🎯 2026-08-14: "Nothing should be marked covered unless it's proven to be. And as items are proven, they should be checked off")*. ⛔ Today's **34 counts DECLARATIONS, not passes** — **10 of them have never executed once**, and flow 08's six were red on the last two runs while still counting. Under the rule it drops to ~**23**. A row is proven when **every** flow claiming it passed; partials count only for their automatable half. ⚠️ **Provenance is preserved, not overwritten:** automation writes `[x] … ✅auto·<runId>` and manages **only** rows carrying its own stamp — it can un-tick a regression but never touches a bare `[x]`, which stays human-earned-on-hardware. ⚠️ Needs a durable results file from the workflow (artifacts expire) → lands with **4.1.9b**'s pass so the workflow is touched once | |
...
| **.7.4d** | **Dispatch and read the verdict** | ▶ **NEXT — uncommitted, ready** |
