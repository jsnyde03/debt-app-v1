...
**✅ 2.13 BNPL-cadence balance-path fix cycle DONE (2026-07-27, `8d79db7`) — verified, awaiting re-scan signoff.** One shared cadence treatment now across every balance/payment-rate surface: AS.1 `projectCurrentBalance` (monthly-equivalent, +biweekly assert) · AS.2 `applyRolloverPayment` WRITE path pays down by the in-window installment count matching the reserved cash — `minimumPayment` untouched, remaining advances (+cross-cadence/aligned/no-window asserts; `payday.ts` passes the window) · AS.3 timeline state-thread by the same in-window count · AS.4 recovery essentials window-scaled · AS.5 amortization uses the monthly-equivalent. **Full `validate:release:rn` GREEN (e2e 28/28).**

**▶ Re-scan gate (Jason: run another scan + get signoff before closing Phase 2).** Adversarial re-check of the AS fixes (correct + complete + all BNPL views now agree for a cross-cadence BNPL, no regression to non-BNPL/aligned) + a fresh sweep for a 6th cadence surface + any regression from the `applyRolloverPayment` signature / `payday` window change.
- **Exit:** re-scan CONSENSUS → **PHASE 2 CLOSES** → Phase 3 opens with the ⭐ Best-in-Class Enhancement audit.
