...
- **Active item:** **▶ 2.7 BNPL as a first-class obligation** — 2.7.1 gate ✅ · 2.7.2 model ✅ · 2.7.3 capture/display ✅ · **2.7.4 Guardian-aware cadence + heads-up ✅ (2026-07-24, e2e 19/19, both themes)**. **▶ NEXT = 2.7.5 BNPL calendar** (the last 2.7 piece; switch-in = design gate w/ Jason). **✅✅ GUARDIAN COMPLETE + AUDIT PASSED. ✅ 2.6 · 2.5 · 2.4 COMPLETE**.
...
**▶ 2.7 ⭐ BNPL as a first-class obligation** — Guardian done + audit-passed; BNPL is the next build. _**Design aligned w/ Jason 2026-07-24: installment-native model + balance fallback; afford-check folds into inverse-Guardian ①.**_
- [x] **2.7.1 [DESIGN GATE] ✅ (2026-07-24)** — before-scan **corrected the stale premise**: `scheduledPaymentAmount`/`remainingPayments` aren't a *conflicting* surface — they're **dead capture** (collected in the sheet, read back nowhere; engine + amortization both run BNPL off `balance`+`minimumPayment` w/ `apr:0`). So 2.7 = make the installment the real model, not "reconcile two surfaces." Design forks resolved w/ Jason (both my recs): installment-native + fallback · afford-check → ①.
- [x] **2.7.2 ✅ (2026-07-24)** Installment-native model — pure `bnplInstallment.ts` (`isInstallmentNative`/`normalizeBnplInstallment`, idempotent) makes scheduled × remaining canonical (balance + minimum derived); applied at all write seams (store add/update · CSV import · **migration v5→v6**) so the engine needs no read-site changes; `syncBnplRemaining` keeps the count truthful at rollover; balance+minimum fallback when installment fields absent. +13 core asserts; tsc/regression/app/scenarios/lint green (e2e → 2.7.3 UI checkpoint). Detail → LOG.
- [x] **2.7.3 ✅ (2026-07-24)** BNPL-native capture/display — DebtSheet gets its own BNPL field set (provider · installment · payments-remaining · cadence · next-date; balance/min/APR derived, not asked) + `bnplProvider?` (additive); DebtRow reads "X of N paid · interest-free" with a provider pill + cadence-aware suffix (`/2 wks` not false `/mo`); demo Affirm upgraded to installment-native. **Cadence coverage (Jason): biweekly→48-mo→one-time via (cadence × count); one-time added; research-all-cadences criterion filed to the Premium audit.** New `bnpl.spec.ts`; full e2e 19/19; both themes verified. Detail → LOG.
- [x] **2.7.4 ✅ (2026-07-24)** Guardian-aware cadence — bounded BNPL-scoped fix: `bnplInstallmentsInWindow` + `scaleBnplMinimumForWindow` scale a sub-cycle BNPL's effective per-cycle minimum to its in-window count, wired at `buildAllocation` (card) + `buildMultiCycleTimeline` (lookahead); paid-flags/rollover untouched. **Surfacing (Jason: minimal named callout): `selectBnplBetweenPaycheck` → a calm all-tier heads-up on the card ("3 Klarna payments … land before your next paycheck").** +18 asserts; both themes verified; e2e 19/19. _(deferred general undercount + BNPL payoff-rate → backlog.)_ Detail → LOG.

**▶ 2.7.5 ⭐ Consolidated BNPL calendar** — the last 2.7 piece: an upcoming-installment view across all BNPL plans (a differentiated, under-served acquisition lever; the "when do my BNPL hit?" answer in one place). _**Switch-in = before-scan (what installment/date data exists per BNPL) + design alignment w/ Jason (surface: a section on Money/Progress? a card? · what it shows: date · provider · amount · "2 of 4" · cadence · between-paycheck flag) BEFORE building.**_
- [ ] **2.7.5.1 [DESIGN GATE]** before-scan the BNPL installment/date data + agree the calendar's home + shape w/ Jason → decompose 2.7.5.2+.
- **Exit:** a coherent consolidated BNPL calendar; verified both themes + tests + e2e.
- [ ] **2.7.5** Consolidated BNPL calendar — upcoming-installment view across all BNPL plans (differentiated, under-served acquisition lever). _("Can I afford this BNPL?" MOVED → inverse-Guardian ①; build that engine once.)_
- **Exit:** BNPL is a coherent first-class obligation (installment-native engine + display · native capture · Guardian-aware cadence · the calendar); verified both themes + tests + e2e.
