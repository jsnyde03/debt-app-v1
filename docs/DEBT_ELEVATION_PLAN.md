...
- **3.5.4.11 — reimagine the demo for the App Preview ✅ DONE 2026-08-06.** Entries un-shipped · the 5-beat arc (Money → Today → Today → Progress → Today) navigating via `DemoDirector` · the closing beat primes a real payoff invitation so the capture driver taps the genuine flow · `?capture=1` strips the chrome. All five beats shot in both themes and reviewed. **2 open, both in the audit doc's capture section:** the debt-free date shifts a year on the closing beat only (Today and Progress AGREE unprimed, so A7 stays closed — `balanceAsOfDate` suspected, unproven), and Skia canvases came back unpainted in one capture-mode frame. ⚠️ **"Likely web-only" was DISPROVEN** by 3.5.8's later cycles — it reproduces on the native simulator, at two consecutive samples, on the beat the video exists for, and the cause is established: `CaptureAutoStart` starts the script on mount while the app paints seconds later, so **the script runs ahead of the screen**. Log: 3.5.4.11.
...
- **3.5.7 — web-embeddable marketing-site demo (pulled IN — no backlog):** an embeddable interactive demo for the marketing site (embed harness + hosting + the privacy call), off the same sandbox substrate + scriptable run. Its own sub-step given the hosting/harness LOE + the App-Preview capture path (3.5.4) it shares.
  - **⛔ Does NOT wait on the Phase-6 device pass (analysed 2026-08-10).** The two share a name — "the demo" — and nothing else that matters here. The **App Preview is a frozen video**, so it must be shot after the UI stops moving; the **embed is live code**, so it redeploys and cannot go stale. And the device pass verifies iOS-native behaviour — Modal presentation, haptics, VoiceOver, safe-area insets, `expo-blur` — **none of which exists in a browser.**
  - **What it actually waits on:** ① the **debt-free-date defect** above, which it shares by replaying the same run · ② 🎯 hosting + the privacy stance · ③ the **web-only `Slider` a11y gap** in the backlog, which becomes real the moment a web surface ships and which `a11y-axe` does not flag.
  - **The one real coupling:** if the device pass finds a defect in the SHARED substrate (the sandbox or the scripted run) rather than in native chrome, it reaches the embed. Unlikely — §12's items are native-specific — but it is the reason to read the device results before deploying, not to gate the build on them.
...
⚠️ **What is genuinely LEFT in Phase 3.5** (audited 2026-08-10, in answer to "what's left?"):

| | Item | State |
|---|---|---|
| **1** | **3.5.7 — the web-embeddable marketing demo** | the only unbuilt BUILD item. Sequenced after 3.7 (🎯 2026-08-10); hosting + the privacy stance still undecided |
| **2** | **The device pass** — checklist §11 · §12 · §13 | owed on the fresh `v1.7-dev` build. This is what "3.5 is device-verified" rests on |
| **3** | **⚠️ The demo's debt-free date shifts a YEAR on the closing beat** | **still open**, `balanceAsOfDate` suspected and unproven (audit doc §"OPEN"). It is the beat the **App Preview** ends on, so a store video carries a number nobody can account for. Same family as the transient `$790` already in the backlog. ⚠️ **Blocks 3.5.7 TOO, not just the capture** — the embed replays the same `DEMO_STAGES` run, so the audit doc's "needs settling before the capture is cut" understates it: unfixed, it shows a prospect a wrong number on the marketing site as well |
| **4** | **The App-Preview asset must be RE-SHOT** | 3.5.8 built the pipeline and cycle 14 is approved, but the submitted file is a re-shoot after the UI settles → Phase 6 |

Items 3 and 4 mean **3.5's OUTPUT is not final even though its build is** — which is why the phase is not signed off.
