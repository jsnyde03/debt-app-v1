...
| order | # | Step |
|---|---|---|
| **1** | **3.5.5.5** | **▶ ACTIVE — ✅ L5 FIXED 2026-08-08** (`footerAccessory` pins the entry below the fields, above the sticky actions; Save separates it from Remove; gate 134/134, guard proven to fire). ⚠️ **The mark itself is PARKED on branch `3.5.5.5-coach-mark` — built, looked at in both themes, and NOT green:** `validate:release:rn` fails under load where an isolated `test:e2e:rn` is 135/135, and a stashed control proves the interference is ours. **Next: replace the 600ms mount-timer trigger** (prime suspect), then re-gate. Three real findings already banked on the branch — log 3.5.5.5 |
