...
| **3** | **⚠️ The demo's debt-free date shifts a YEAR on the closing beat** | **still open**, `balanceAsOfDate` suspected and unproven (audit doc §"OPEN"). It is the beat the **App Preview** ends on, so a store video carries a number nobody can account for. Same family as the transient `$790` already in the backlog |
