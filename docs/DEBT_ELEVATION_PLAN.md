...
| ▶ **S1.11.4.2** | **THE MOUNT POINTS.** ✅ **`C4-9`** *(blocker)* — the ring credited **$12,000 the user had not paid**, its only trust check sat in the `!hasDebts` branch; four figures suppressed together, e2e now runs the mixed portfolio. ✅ **`C4-7` + `D3-2`'s `PARTIAL`** *(blocker)* — the Lock Screen and Siri refused while Today said *"the spare **$1,800**"* against a true **$300**; the verdict goes with the figures, and `requiredPlanTrust.test.ts` now walks **six surfaces in ONE list**. **Remaining: the `PARTIAL` `C-4`, whose open half IS `C4-2`** *(struck from `S1.11.5`, not done twice)* | every surface that states the claim in ONE list, and the assertion walks the list |
