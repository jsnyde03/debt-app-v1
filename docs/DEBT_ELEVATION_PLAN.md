...
| **.12.6.6** | **MONEY WRITTEN OR DESTROYED** — `B2-1` · `B2-3` · `B3-2` · class-5 R1 `L2-4` *(the foreground drain destroys a queued Siri payment)* *(`B1-2`, a blocker, closed early at `.5.7` ④b)* | 12 | every money write through one normaliser, asserted over the actions |
...
| # | step | exit line |
|---|---|---|
| ✅ **.5.7** | **CLOSED 2026-09-14 — BOUNDARY + LEDGER DRAIN.** Native flows and four instruments ①② · render proofs ③ · ④a a replayed queued intent applies once, and pass-6 `C3-6` **reopened and closed** *(the Lock Screen tap names its payday)* · ④b three surfaces promising money the engine will not hold, fixed *(`B1-2` a blocker, from class 6)* · ⑤ `hero-date-fit`'s host-dependent red was a real web clip, fixed regardless of font · ⑥ plan cleanup · ⑦ **one registration batch of 14, one drain: 26 of 26 proofs MATCHED**, ledger green at stale 2. 🎯 decisions and every measurement → log | ✅ |
| ▶ **.5.8** | **`[D79]` RE-AUDIT of class 5 — fresh agents, cumulative, never the fixer.** ✅ **8.1** [`CLASS5-REAUDIT-BRIEF.md`](audits/2026-09-02-s1-money-pass7/CLASS5-REAUDIT-BRIEF.md) — range corrected to `c7df99c2..` *(the handoff's covered `.5.7` only)*, 72 files in 4 generated lanes · ✅ **8.2** 4 lanes run, 24 findings · ✅ **8.3** [`CLASS5-REAUDIT-ROUND1.md`](audits/2026-09-02-s1-money-pass7/CLASS5-REAUDIT-ROUND1.md) — **17 attributable, 7 reproduce**; 🎯: the 5 unreached surfaces stay in class 5, `F1` `F3` `F5` move on → **19 to fix** · ▶ **8.4** round-2 fixes, by file: ✅ **8.4.1** 2026-09-14 PlanHero `L3-1a` `L3-1b` `L3-4` + `FX-1` `FX-2` found at switch-in — one claim per hero figure family, APR routed on avalanche (🎯), 11 plants MATCHED; [evidence](audits/2026-09-02-s1-money-pass7/class5-round2-fixes/) · ✅ **8.4.2** 2026-09-14 Progress `L3-5a` `L3-5b` `L3-2` `L3-6` — cash flow asks `'solved-projection'`, the tier lives in `'projected-balance'`'s route (🎯), sweep is tier × strategy; 8 plants + a 15-proof drain MATCHED · ✅ **8.4.3** 2026-09-15 plan repairs + line `L1-1` `L1-3` `L1-4` `L1-5` + `FX-3` `FX-4` found at switch-in — app-written plan fields unrecoverable (🎯), setters answer at `$0`, `computeState` honors a set `$0` line (🎯); 18 plants + a 16-proof drain MATCHED · ▶ **8.4.4** liveness vs ranking `L1-R1` `L1-2` + `FX-5`: one `payoffOrder`, a `$0` estimate listed last, never the focus (🎯 ×2) — code `da1280fb`; **close owed:** run-3 e2e plants · full `lint:rn` · drain 29 stale *(START-HERE "Resume here")* · **8.4.5** outside the app `L2-1` `L2-2` `L2-3` · **8.4.6** instruments `F4` `F6` · **8.4.7** drain, then register `F2` + every proof 8.4.1–.6 wrote · **8.4.8** boundary, push, CI · **8.5** round 2 by fresh agents; repeat until a round finds zero defects attributable to class 5's fixes | 0 new defects attributable to class 5's fixes |
