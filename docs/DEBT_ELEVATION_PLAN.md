...
| # | sub-step | exit line |
|---|---|---|
| **.12.1** | **BUILD THE DESTINATION** *(🎯 2026-08-31 "measure, do not audit")* — ⚠️ **rewritten by `.12.2`'s measurement.** Not 197 files to classify: **86 already-classified files routed to `s2`(1) / `s3`(39) / `s4`(46), surfaces that have no claims file.** The classification exists; the destination does not | s2/s3/s4 carry claims files, and every routed file lands on one |
...
| ▶ **S1.13.7.12** | **INSTRUMENT THE REST OF THE TREE, THEN PASS 7** *(🎯 2026-08-31)* — ⚠️ **re-measured 2026-09-02: 209, not 197, and 123 of those are trees the completeness gate already skips by name.** The real population is **86 files routed to `s2`/`s3`/`s4`, which have no claims file.** ⛔ **Measure, do not audit.** Decomposed above | 86 | every routed file on a claims file |
