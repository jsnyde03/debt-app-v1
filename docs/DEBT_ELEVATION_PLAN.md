...
⛔ **[D42] is a BAR, not a count:** all 62 **judged**; **fixed** = every defect + every finding on a
shipping surface. ⭐ **Triaged 2026-08-20: 37 fix · 25 not work.** ⚠️ **The "5 die with the `QA_TOOLS`
flip" line was WRONG and is gone** — it describes the strings instrument's 61 duplicates, a different
population; **no id here dies with the flip.** Ledger, the 9 corrected verdicts, the half-closed pattern
and the L2-10 divergence → log.

⚠️ **Two the triage must not get wrong:** **L6-7** is a *publishable* RevenueCat key and is **already
closed**; **L5-21** records that "no loading state on native" is **correct** — "fixing" it is the defect.

| # | Sub-step |
|---|---|
| ✅ | **P6.4.1–.3 DONE 2026-08-20** — the triage *(re-run once; the first pass read the CODE and never the LEDGER and was wrong 9 times)*, then **two empty clusters**: claims and number-formatting were already closed at T5.2 / T6. What the triage found in their place: **`lint:money` green over 5 hand-rolled money sites**, one handing VoiceOver `"$5000"` → TS AST pass added, both new detectors planted. Plus **L4-15** → `selectLivingReserveRequest` and **L4-14**'s docstring. Detail → log |
| ▶ | **P6.4.4 — copy + duplicates.** ✅ **Part 1:** L1-28/L5-18 · L1-32 · L1-35 — ⛔ **two Maestro flows pinned the retired string.** ✅ **Part 2:** L2-10 — 🎯 reframed it; R3 had fixed only what the exit *said*, so the explore exit is now a `Pill`, and `Pill` gained `testID` + `hitSlop` (32 → 44pt, the open `hitRegion` class). ✅ **L1-22 → P6.8 with `lint:apostrophes` built** (73 sites, not the 152 first quoted). ✅ **Part 3:** **L6-6** was not copy at all — a control-flow **sentinel compared in 8 places that FAILS OPEN**, never rendered; now `DEBT_FREE_DATE_UNPAYABLE` *(and deliberately **not** in `vocabulary.ts` — that is L2-6's refuted move)*. **L1-24** → "Not with minimums"; its "one condition" premise **measured false**. ✅ **Part 4 — the L2 shared-constant tier:** L2-11/12/15/17/20/21/23 → 7 owners in `vocabulary.ts`, **judged one at a time against L2-6's refutation, not batched**. ⚠️ **L2-12 is NOT one constant for four sites** — a sheet title is a noun phrase and its submit button an imperative; the drift was the two *entry points* disagreeing. ✅ **Part 5:** L1-21 *(sentence case; **3 Maestro `tapOn`s + a route-smoke pin** moved with it)* · L1-27 *(the four hype outliers — ⛔ **"Debt-free!" is compared in `SiriQueryIntents.swift`**, which a TS-scoped sweep cannot see)* · L1-31 · L1-33 · **L2-19** *(forced by the gate: sentence-casing the history title CREATED the row-vs-destination duplicate, so it was extracted, not baselined)*. ⛔ **L1-30 is MOOT — verified, not inherited:** its five labels are `cushion_buffer`/`expense_reserve`/`prefunded_reserve`/`discovery_holdback`/`true_leftover`, none surfaced by `selectActiveRecommendedActions` — L2-6's five dead strings. ⛔ **L1-20 → P6.8:** its premise is inverted — **only 2 of 8** eyebrow styles have `textTransform`, so this is a re-implementation (16 literals + 6 styles + **26 test pins**), not a cleanup. ✅ **Part 6 — CLOSED:** **L2-13** *(⚠️ its site list conflated two fields — the real defect was both screens offering the SAME example balance with different minimums, $65 vs $35)* · **L2-18** *(`PAID_OFF_LABEL` / `SHARE_WIN_CTA`; at 8 chars "Paid off" is below `lint:copy`'s floor, so the gate was blind and `ShareCard` is what a user posts publicly)* · **L1-25** *(a platform-neutral `notificationCopy.ts` both the native module and the web stub import — the stub's contract is "the same surface", and it had drifted into a different voice)*. ⛔ **L1-23 REFUTED, recorded at the site:** its four "captions for one state" are four **states**, one of which is an affordance — `money.tsx` says "tap to verify" *because there the caption is tappable*, so the suggested single caption would tell users to tap something inert everywhere else |
| ✅ | **P6.4.5 DONE 2026-08-20.** Both real defects fixed: **L5-13** *(a debt-free user now gets "Every balance cleared" instead of "$0 · remaining across 0 debts" over an inert strategy toggle — Money had been contradicting Progress one tab away, on the same store)* and **L6-9** *(`TextField` gained a neutral `note` channel; the finding's hypothesis was **measured** — `error` paints border AND caption `c.accent.danger`, so the app was using the failure treatment to say an entry was fine while its submit button stayed enabled)*. Plus L5-16/17/20 defensively *(rendering still unmeasured → P6.14 rows)*, L6-8, L4-12, and **L5-19's code half** — `introPrice` was never modelled, so an ASC-configured trial would apply at purchase and appear **nowhere**. ⛔ **Found while there: L1-27 had HALF-closed itself** — the price-minimising *"just"* survived on the LIVE `planFromPackage` path after being removed from the static fallback. ⚠️ **L4-13 is half-done by design:** the `pressedOpacity` token landed (six values measured); *"`PressableScale` app-wide or nowhere"* is a visual-system call → **P6.8** |
| ✅ | **P6.4.6 CLOSED — zero edits, by design.** It converts to a **P6.11 obligation**: L0-4 is a null *(both files deleted three weeks BEFORE the audit; a zero-reference check cannot tell dead from absent)*, and **L3-5 · L4-11 · L6-4/5 all have live ROOT-tree consumers**. ⛔ L3-5's SYNTHESIS refutation had the tree backwards — the RN app never imports `buildSmartInsights` |
