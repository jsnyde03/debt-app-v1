...
| # | sub-step | exit line |
|---|---|---|
| ✅ **S1.13.7.9.1 + .9.2** | **ROOT CAUSE FOUND BY BISECT — `2c77aceb` (`S1.13.7.5 wip`).** ⛔ **`C1-18`, a MINOR about a literal declared twice, is what produced the red suite.** Its remedy made `CushionBarChart` the owner of `RESERVE_OPACITY` — and that module imports `@shopify/react-native-skia` **at module scope**, so `PaydayGuardianCard` importing one number from it put **CanvasKit in Today's import graph for every render**. Today threw `…reading 'XYWHRect'` and rendered an **empty body**: the swipe actions, the tutorial fence and every click target were simply absent. ⚡ **And the earlier "multi-cause" reading was itself wrong** — `expense-reserve` went green with this fix too; flipping the shared fixture's dates had only changed timing enough to mask it | CLOSED |
| ▶ **S1.13.7.9.3** | **THE FIX IS IN; CI IS THE ARBITER OF WHAT IS LEFT.** Token re-homed to `theme/colors.ts`, which both sides import. **Measured: `swipe-mark-paid` + `a11y-axe` + `expense-reserve` 20-failing → 24/24, and the embed suite 9/1 → 10/10.** New guard in `lint:import-graph`, proven: **a plain VALUE may not be imported from a module that loads a renderer**, discriminated by PascalCase — a screen importing `<TrajectoryChart/>` is rendering a chart and pays deliberately; a number must never cost CanvasKit. ⛔ **The local full-suite number after this fix is NOT usable — 47/292 with 60s `page.evaluate` timeouts, a machine-under-load signature** *(the prior full run was 302/37 in 29m; this one followed `lint:rn` back-to-back)*. **Read CI, not this box** | CI's e2e failure list, re-measured against the fix |
| **S1.13.7.9.3b** | **THEN FIX WHAT CI STILL REPORTS, BY CAUSE** — including `hero-date-fit`'s 320pt clip if it survives, which is a real user-facing defect on the app's headline number | each cause closed once, with a plant proving the red returns |
...
| ✅ **S1.13.7.9.6** | **[DECISION] ANSWERED 🎯 2026-09-01 — *"I agree. Let's push where appropriate."*** ⛔ The gap was never a missing check: CI already runs every suite [D74] names, on every push. **It was 36 unpushed commits.** ⚡ **A push is not a build dispatch** — `batch-ci-builds` governs what you DISPATCH (a Codemagic native build, a manual run), never `git push` to a branch whose CI is automatic and free. **The batch boundary is the item close, and the item close now ENDS WITH A PUSH**, with the run read by `gh run view --json conclusion`. ⚠️ Holding a push back does not save a spinner — it hides a red one | ANSWERED |
