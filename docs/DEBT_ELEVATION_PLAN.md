...
| ▶ **S1.13.7.10.5** | **THE REMAINDER — 4 of 14 closed.** ✅ `A1-1` + `A1-2`: `scenario()` type-checked **nothing**, so a pref's wrong SHAPE was invisible to `tsc`, the linters and the run — `coachMarksSeen: true` (`.includes` on a boolean is a `TypeError`) and **40 seeds of `guardianIntroSeen`, a pref v7 deletes**. `over.prefs` is typed now; both plant back red. ⚡ **And the type found a defect no lane reported** — `tutorialSeen: false` where the docblock says *"deliberately not a boolean"*, and the consumer tests `=== null`, so it means *"will NOT be offered"*. ✅ `A1-10`: a test that manipulated the dead pref **and** asserted a string absent from the whole app — deleted, its subject removed by 5.6. ✅ `A1-6`: the `prefs` merge was discarded by the `...over` that followed it. ▶ **Left: `A1-3` `A1-7` `A1-8` `A1-11` `A2-7` `A3-4` `A3-14`–`A3-16` `D1-6`** — mostly the *one-arity* shape | closed, or measured never to have been a finding |
