...
| ▶ **S1.12.9** | **BUILD S1.12.7** — un-clamp the ledger's running balance in its two components + a `< 100` colour threshold; delete the clamp's now-false premise rather than annotate it [D17]. Plant: re-clamp → the row must red | the true number on screen, guarded |
| **S1.12.10** | **BUILD S1.12.8** — `reserveHeldAt` stamped at hold time, migration fallback to the current cycle's start, `Math.min(surpriseSum, heldReserve)` kept. Plant both directions: credit-all and credit-none | the window is the hold, guarded |
| **S1.12.5.9** | **THE NET.** typecheck ×4 · `lint:rn` **42/42** · three unit suites — green. ⛔ **e2e is 337/1: `vis5-cone` failed in the full suite and PASSES 2/2 alone** — order- or timing-dependent, being re-measured, **not** filed as flake. ⛔ **No `gate:record`** — [D74] writes it at convergence, and pass 5 did not converge | net green and pushed |
