...
| **P6.4.5** | **States + rows — L5-13/16/17/18/19/20 · L6-8/9 · L4-12/13.** ⭐ **The two real defects: L5-13** *(all debts paid → the hero reads "$0 · remaining across 0 debts" over a strategy toggle and an empty list; the empty state gates on `debts.length === 0` and a cleared debt stays in `debts`)* **and L6-9** *(an informational message rides the `error` prop — **measured**: `TextField.tsx:65,69` paints border AND caption `c.accent.danger`)*. ⚠️ **L5-16/17/20 are style reads with UNMEASURED rendering** — fixes are cheap and defensive, the proof is a P6.14 device row |
| **P6.4.6** | ⛔ **ZERO edits — it becomes a P6.11 obligation.** L0-4 is a null *(both files deleted three weeks BEFORE the audit; a zero-reference check cannot tell dead from absent)*; **L3-5 · L4-11 · L6-4/5 all have live ROOT-tree consumers.** ⛔ L3-5's SYNTHESIS refutation had the tree backwards — the RN app never imports `buildSmartInsights` |
