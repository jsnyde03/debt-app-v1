...
| ▶ **.5.5** | **`B1-1` — save-for-it promises a pace and a ready-by the engine cannot fund.** ⛔ **Switch-in: wider than filed, and the finding's remedy is also broken** — the engine clamps to what is left after the buffer and reserve, so `Save fast` breaks on **4 of 4** shapes and `Balanced` on 2, and pacing off spendable still promises too much. ① ✅ switch-in, measured · ② one producer: the engine's funded share for the prospective goal, pace unset — both dated options derive from it, rounded down, with the date · ③ asserted per shape (debts · an existing goal · variable income) by name with a control; planted · ④ close, push | every dated option the sheet offers is what the engine funds once stored |
