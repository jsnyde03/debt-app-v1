...
| **T5** | ✅ **BUILT 2026-08-14 — and it found a SHIPPED defect on its first run.** ⛔ `DemoDock.tsx:45` — `<View accessible>` (there to satisfy **§12.6.4**'s "reads as ONE utterance") contains **both dock exits**, the `Button` at :73 and a `Pressable` at :74 — which is exactly what **§12.6.3** requires VoiceOver to reach. **The app satisfies one checklist row by breaking the other**, on the surface that feeds the App Preview and 3.5.7's public embed. 🎯 **[DECISION] — one utterance vs reachable exits is a product call, not mine.** ⚠️ `lint:a11y-collapse` runs standalone and is **deliberately NOT in `lint:rn` yet**; it joins the gate when this resolves. Proven both ways first (fires on `accessible={true}`, clears on `={false}`, exactly one finding). *(was: `lint:a11y-collapse`* | |
| ~~T5~~ | *…the original filing:* — flag any JSX element carrying `accessible` that is **not itself pressable** but contains a `Pressable`/`Touchable`/`Button`. iOS collapses such a subtree into one element and the control **ceases to exist** — measured 2026-08-13: the coach-mark card was a leaf with `children: 0` and **VoiceOver could not dismiss the hint.** Needs the TS AST (a text scan cannot see structure); `strings-inventory.ts` established that machinery. ⚠️ Prove it fires on a planted defect before trusting it, per T2 | 📋 **FILED 2026-08-13** (🎯 not now — CI green first) |
