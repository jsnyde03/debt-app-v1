...
###### S1.13.7.10's sub-steps — the ACTIVE decomposition *(the ONLY one on this doc)*

⛔ **THE RULE THIS CLASS ADDS: A TEST'S FIX IS PROVEN BY MAKING IT RED, NOT BY MAKING IT GREEN.** Every
finding here is an assertion that already passes, so a repair that leaves it passing has demonstrated
nothing — **the plant is the entire proof.**

⚠️ Otherwise the usual loop: verify the MECHANISM against current code → reproduce with a control → fix →
re-run the control → plant the original defect and confirm the red is FOR THE PLANTED REASON → plant the
laziest plausible repair → register the guard. ⛔ **A remedy is a HYPOTHESIS** — `S1.13.7.9` refuted two of
its own in one sitting.

| # | sub-step | exit line |
|---|---|---|
| ✅ **S1.13.7.10.1** | **CLOSED — THE CLASS IS 19, NOT 18.** Counted from the lane files: `CLASSIFICATION.md`'s membership line names **19** ids and its header says 18. All 19 have a real `##` heading — none phantom, none missing. ⚡ **The undercount class, on the count of the undercount class.** ⚠️ Also measured and FILED rather than fixed: **CLASS XI claims 43 minors and holds 40**, and the header's `29 + 48 + 47` sums to **124** against 123 findings — the class table only balances because IX's −1 and XI's +3 partly cancel. → backlog, `S1.13.7.11`'s scope | CLOSED |
| ✅ **S1.13.7.10.2** | **CLOSED — `D1-5` + `A1-9`, both proven by plant.** ⛔ **`D1-5`'s implied explanation was wrong and it changed the fix**: the engine does not emit a snowball the timeline drops — `buildTimelineItems:129` emits extra payments **only from `completedRecommendedActions`**, so `-1` was correct for a fixture that passed none. The three `!== -1` escape hatches are unconditional vacuity floors now. ⚠️ **And the finding overstated its blast radius** — `testFullAppRegression` already pinned snowball-after-*minimum_debt*; what was genuinely uncovered is snowball-vs-**expenses**, invisible there because it passes `requiredExpenses: []`. ⚡ `A1-9`: `.first()` resolved to the *"Your plan"* row, which emits the same words from the same helper **at rest** — the What-If row has its own testID now, and **the control (absent before typing) is the half that makes it an assertion** | CLOSED |
| ✅ **S1.13.7.10.3** | **CLOSED — `D1-4`, proven by plant.** ⛔ An `Invariant` returned `null` for *"it holds"* **and** for *"the field I need is absent, so I checked nothing"* — opposite facts the harness could not tell apart, in the instrument whose job is *"can the migration lose data?"*. A `SKIP` sentinel splits them; `checkAll` filters it exactly as it filtered `null` so no caller changes; and the harness now accumulates per-invariant evaluation counts against a recorded downward-only floor, **printed every run**. ⚡ **Now visible: `nothingSilentlyDropped` evaluates 542 of 1,084 BY DESIGN** — `importDoor` supplies no `accounting` at all, and half looks identical to none until somebody writes the half down. ⚠️ The self-check tested `!== null` and read `SKIP` as a violation on the first run — taught, not worked around | CLOSED |
| ✅ **S1.13.7.10.4** | **CLOSED — `A3-17` + `B2-4`, both now driving the real producer.** ⛔ **`A3-17`'s eight fake assertions were sitting on a REAL hole**: `backup.test.ts` had 48 assertions and mentioned `completedRecommendedActions` / `isPaidThisCycle` / `minimumPaidThisCycle` **zero** times — the exact three they named. Real round-trip coverage added first *(48 → 71)*, then the eight removed with a pointer. ⚡ Proven by a real regression: dropping `completedRecommendedActions` from `serializeBackup` reds the new coverage while `test:regression`, where the eight lived, **exits 0**. ⛔ **`B2-4`: all four delete actions now run against a wired store** — pass 5's `$10,967.54` blocker verified through the action instead of a hand-built object. ⚠️ **My first draft of that test was wrong and failed at once**: an unreferenced id IS free to re-issue, because `reservedDebtIds` reads the whole document — that direction is now the control, and it is what proves the set is not a counter | CLOSED |
| ✅ **S1.13.7.10.5** | **CLOSED — 13 of 14, and `A2-7` deliberately left OPEN as a [DECISION].** ⛔ **`A3-4` exposed a live MONEY defect**: on one weekly debt, RESERVE **$50** against PAYDOWN **$200** — `A3-1` argued its widening was safe because *"the allocator's RESERVE and the PAYDOWN both read `effectiveMinimumInWindow`"*, and **the allocator did not read it**. The reserve reads the shared producer now. ⚡ Also: `A1-1`/`A1-2` — `scenario()` type-checked **nothing** *(40 seeds of a pref v7 deletes; a `tutorialSeen: false` no lane reported)* · `A3-15` — the partition test derived its subject from its object · `A1-7` — the render barrier was the **tab's label** · `A1-8` — the band count was an unanchored substring test, and fixing it **fires a real a11y warning** · `A1-10` — a test asserting a string absent from the whole app. ⚠️ `A3-16` annotated, not deleted *(class X owns that tree)*; **`A2-7` stays open** — its own remedy forbids the test without an engine decision, and the measurement is filed | CLOSED (13 of 14) |
| ✅ **S1.13.7.10.6** | **CLOSED — the 5 proofs this class made stale, re-run and all held.** Stale 5 → 0. ⚠️ Not `--all`: the proofs whose targets this class touched are exactly the ones the ledger reports stale, so that IS the population | ~~RE-RUN THE EARLIER CLASSES' GUARDS THROUGH THE REPAIRED INSTRUMENTS~~ — ⛔ this is why the class is last: a guard registered in `.1`–`.9` was proven by an instrument this step repairs, so `prove:guards --all` afterwards is the only thing that says those proofs still hold | `prove:guards` green across the round, stale at 0 |
| ✅ **S1.13.7.10.7** | **CLOSED — typecheck ×4 · `lint:rn` 45/45 · three unit suites all green, pushed.** The e2e half runs in CI, per `S1.13.7.9.6` | ~~THE NET, THEN PUSH~~ — typecheck ×4 · `lint:rn` 45/45 · three unit suites · full e2e · embed, then **push and read CI** *(`S1.13.7.9.6`)*. ⛔ No `gate:record`: [D74] writes it at convergence, and pass 6 has not converged | CI green on the pushed sha |

**Exit (S1.13.7.10):** every assertion in the round's instruments reachable and falsifiable, every guard
from `.1`–`.9` re-proven through them, and CI green.
