...
- **▶ ACTIVE BUILD: 3.5.5 — feature-discovery coach-marks,** decomposed below. .1 (primitive + registry app-wide) and .2 (one-interruption-at-a-time, re-scoped) ✅ done 2026-08-07; **.3 → .6 remain.** ⚠️ **Start .4 with a FRESH read of the code** — its whole job is verifying each affordance is reachable, and its parked inventory has already been wrong once (it promised an income-varies toggle that did not exist → 3.7.A9).
- **🔴 3.5.8 — CYCLE 8 REVIEWED 2026-08-07, and it FAILS on content, not on mechanics.** The three cycle-7 fixes all worked (contact sheet renders · 8 of 10 frames · the `$1,747` did not reproduce, beat 5 reads `$2,000`). But the frames show **black at the in-point, an EMPTY Today body for beats 2–3, and Progress with its Skia geometry absent** — byte-identical pairs, so held blankness, not mid-animation. **One cause:** `playDemoRun`'s clock starts at mount, so the script runs ahead of the screen; `LAUNCH_ALLOWANCE` is estimating a quantity that moves (cycles 7 and 8 measured paint ~3s apart on the same build). ⚠️ The "unpainted Skia is web-only" hypothesis is **disproved**. → **3.5.8.9**, decomposed below. Detail → log 3.5.8 cycle 8 + the audit doc's cycle-8 section.
- **▶ 3.5.8.9 — the anchor is FOUND and holds (cycle 9, 2026-08-07).** `LAUNCH_ALLOWANCE` deleted; all 10 frames extract; beats land at their declared times. Cycle 9's miss was the slate firing before the window was composited (350ms held, 205ms recorded) — now gated on `AppState`-active + a settle, and the shortfall is asserted. **Remaining: cycle 10 (3.5.8.9.6).**
- **🎯 Two decisions still owed:** the payoff-confirmation tap (3.5.8.6b) and draft-vs-submitted (3.5.8.8) — ⚠️ **.8 is not decidable from cycle 8's evidence**; `warm.png` is a settled frame, not an arrival frame. The next cycle's 2fps sheet is what makes it decidable.
...
- **3.5.5 — feature-discovery coach-marks · ⏸ PAUSED mid-flight for 3.5.8.9.** .1 (the primitive + the registry at the ROOT) and .2 (one-interruption-at-a-time, re-scoped to the narrower true claim) ✅ done 2026-08-07. **.3 seen-persistence + replay · .4 the corrected inventory · .5 payoff-schedule L5 · .6 verify+close** remain — decomposition and the switch-in re-verification → log 3.5.5. ⚠️ **Resume .4 from a FRESH read of the code:** its parked inventory has already been wrong once (it promised an income-varies toggle that did not exist → 3.7.A9, now built).
...
### 3.5.8 — the App-Preview capture pipeline · 🔴 CYCLE 8 REVIEWED — one defect, one fix owed

One dispatch on the free GH-macOS lane produces the submittable file. **[D20]:** the build starts itself (`EXPO_PUBLIC_CAPTURE_DEMO`), `simctl` records, ffmpeg conforms. Dark mode ([[feedback_dark_mode_screenshots]]); tag-triggered so nothing touches `release/v1`.

**Apple's rules — RE-VERIFIED 2026-08-06 against ASC Help:** 15–30s · **max** 30fps · ≤500MB · H.264 10–12 Mbps or ProRes 422 HQ · audio optional · in-app footage only, no fingers/device frames · IAP/subscription features must be disclosed · ≤3 previews per language. ⚠️ **iPhone takes ONE fixed 886×1920 file for every modern slot** — not a simulator resolution (the sim records 1320×2868, measured), so the conform is load-bearing.

✅ **3.5.8.1–.7 DONE (2026-08-06 → 08-07)** — the persona's weight + the rent finding · the closing caption ([D20a]) · the capture-flag entry (two cycles proved `simctl openurl` cannot work: iOS confirms custom-scheme opens even when the app is frontmost) · the debt-free-date guard · the conform script with ASC-spec assertions · the workflow + distinct-frame guard + measured `blackdetect` anchor · the web reference set (`npm run shots:demo`) which found **C1's cents root**. **3.5.8.4b ANSWERED: native Skia paints, and better than web** — the unpainted-canvas defect was web-only CanvasKit. Detail → log 3.5.8.x.

✅ **3.5.8.7n DONE 2026-08-07 — every beat's true first frame verified, and three of five fail.** The step did its job: it is the reason the defect below is known rather than shipped.
...
| 3.5.8.8 | 🎯 **[DECISION] draft-vs-submitted** — sim capture ships as the asset, or a real-device re-run does. ⚠️ **Blocked on 3.5.8.9's cycle:** if the arrival lag is runner starvation it vanishes on a device; if it is the per-stage re-seed it does not, and that is the fact this decision needs |

#### 3.5.8.9 — start the clock at PAINT, not at mount ▶ ACTIVE BUILD

The one cause under the black in-point, the empty Today and the geometry-less Progress. **Slate chosen (Jason 2026-08-07)** over settle-only and log-timestamp: white is unambiguous against a near-black theme, it is a fact the app *asserts* rather than one inferred from pixels, and it is trimmed out by construction.
...
| 3.5.8.9.5 | **Cycle 9 reviewed 2026-08-07 — anchor HOLDS, slate fires too soon.** `T0` found; all 10 frames extracted; beats land at their declared `at`. But `beat-1-FIRST` is the iOS launch animation: the app held the slate 350ms and only 205ms was recorded, so it fired before the window was composited. Fixed — `AppState`-active gate + an 800ms compositing settle, the recorded-vs-held slate length now asserted, FIRST offset +0.20 → +0.80. Log: 3.5.8 cycle 9 |
| 3.5.8.9.6 | ▶ **Cycle 10, then verify** that `beat-1-FIRST` is a painted Money screen. ⚠️ Cycle 9's runner painted ~1s after launch vs cycle 8's 8–11s, so a fast cycle does NOT settle the arrival lag — that, and 3.5.8.8, still need a slow run or a device |
