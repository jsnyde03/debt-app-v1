...
- **🔵 FIRST THING NEXT SESSION — capture cycle 8** (`app-preview-20260807-08`, run 31202129089) was in flight at session end. **Download the artifact and look before doing anything else:** confirm `beat-1-debts-FIRST.png` is **Money** (cycle 7's was Today → allowance was too late, now 9→5), that all **5** settled frames extracted (cycle 7 got 3; `-ss` moved after `-i`), and that `contact-sheet.jpg` exists (cycle 7's `drawtext` needed libfreetype). If those hold, 3.5.8 is done pending Jason's two decisions. ⚠️ **Also unexplained:** cycle 7's beat-2 frame read **$1,747** where every web reference shows the persona's **$2,000**.
...
### 3.5.5 — feature-discovery coach-marks ▶ ACTIVE

Calm · one-at-a-time · dismissible · replayable · iOS-16-safe · rendered OUTSIDE gesture handlers · platform-gated. **Scope (Jason 2026-07-31): the tutorial stays Guardian-only; coach-marks teach the rest.**

**Switch-in re-verified against current code 2026-08-07** (the parked decomposition dated 2026-08-06 — detail → log 3.5.5): **3.5.5.1 is de-risked** — `store/tutorialTargets.tsx` was built for this by design ("3.5.5 needs the same thing… ids are free-form strings and nothing here knows about beats"), so the work is mounting its provider app-wide **without losing its inert-when-no-tutorial guarantee**, not building a second measuring system. ⚠️ **3.5.5.2's premise has DRIFTED** — the "VIS-4 single-ack slot" is a **Today-local ternary chain**, not an app-wide coordinator, and its own comment records that the ranking "only governs the FALLBACK now" since the tutorial invite moved out and can already render beside an ack. Coach-marks live on Money/Progress/More, where that slot has no reach. ✅ **The "income-varies toggle" now EXISTS** — it did not resolve at the 2026-08-06 scan, which is what surfaced **3.7.A9**; A9 built it, so it is markable.
...
### 3.5.8 — the App-Preview capture pipeline · ⏳ AWAITING REVIEW (build complete)
...
**Remaining — all review, no build:**

| # | Step |
|---|---|
| 3.5.8.7n | **Verify every beat's TRUE first frame** from cycle 5's artifact, now that the anchor is measured (cycles 1–4 extracted the wrong frames or none) |
...
| 3.5.8.8 | 🎯 **[DECISION] draft-vs-submitted** — sim capture ships as the asset, or a real-device re-run does. Apple's wording is "footage captured on device", but the simulator's Skia fidelity is now proven good |
