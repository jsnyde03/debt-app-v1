...
| **S1.13.7.10.6** | **RE-RUN THE EARLIER CLASSES' GUARDS THROUGH THE REPAIRED INSTRUMENTS** — ⛔ this is why the class is last: a guard registered in `.1`–`.9` was proven by an instrument this step repairs, so `prove:guards --all` afterwards is the only thing that says those proofs still hold | `prove:guards` green across the round, stale at 0 |
| **S1.13.7.10.7** | **THE NET, THEN PUSH** — typecheck ×4 · `lint:rn` 45/45 · three unit suites · full e2e · embed, then **push and read CI** *(`S1.13.7.9.6`)*. ⛔ No `gate:record`: [D74] writes it at convergence, and pass 6 has not converged | CI green on the pushed sha |
