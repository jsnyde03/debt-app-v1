...
| **.7.4h** | **Map the a11y audit instead of guessing at it** — per type, timed, each caught separately | ⏳ **BUILT 2026-08-14**, dispatched **combined with flow 10's `repeat` fix** (`scope=full device=iphone`) so one ~30 min cycle answers both. ⚠️ Swift cannot be compile-checked off a Mac; the compile is `continue-on-error`, so a bad case name costs the map, not the flows |
