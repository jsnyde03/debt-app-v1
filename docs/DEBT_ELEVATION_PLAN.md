...
| **.5** ▶ | `PARTIAL` — the legacy-`0` stand-down reports nothing | ✅ **Premise RE-MEASURED and it holds**: a store already carrying `priority: true` + a pace of `0` is stood down at `migrations.ts:250-278` with **no `pendingDataRepairs` entry and no card** — the silent drop this module's own opening rule forbids. `DataRepair.kind` (new at `.11.12.1`) is what lets a record say *lost* without dating it to today |
| **.6** | `PARTIAL` — `runMigrations` throws on a `null` goal row | ⛔ **ALREADY FIXED by `.11.12.2`** — `repairMoneyFields` drops and reports a non-object row at the seam all four lists share. ⚠️ **But the pin is one member of the class**: `readBackup.test.ts:165` poisons `debts` only. **This step is the FIXTURE**, not a fix |
| **.7** | `PARTIAL` — the attribution test cannot reach `readLegacyStores` | An instrument that cannot reach its subject — `.11.12.11`'s class in the unit layer |
| **.8** | `PARTIAL` — `DataRepairsCard`'s copy promises a route that does not exist | The only one of the four a user meets directly. Resolvable once `.4` builds the route |
