...
##### S1.11.6's sub-steps — the route, fixed before it is trusted

⛔ **The checker comes before its output.** `D4-11` is `audit-route.ts`'s own unreachable self-checks and
two constant numbers in its success line; every later step here reads that line, so it is repaired first.
⚠️ Every premise below is a **hypothesis until measured** — this round ran at 6 of 11 remedies wrong, and
two findings whose named defect was already closed.

| # | sub-step | exit line |
|---|---|---|
| ✅ **S1.11.6.0** | ⛔ **CI HAD BEEN RED FOR SIX PUSHES AND SIX SESSIONS REPORTED THIS GATE GREEN.** `S1P3-M7`'s proof anchors carry literal CRLF — they match a Windows tree and **0×** in CI's LF checkout, and `lint:finding-guards` then says *“the proof is VOID”* about a proof that is fine. **12 entries carry a multi-line anchor.** Third instance of *green where it is run, red where it matters*; closed by picking ONE world (`lib/anchor.ts`). ⭐ The corrected gate **reproduces CI's failure locally**, which the original could not | the gate means the same thing on both platforms |
| ✅ **S1.11.6.1** | **`D4-11` CLOSED.** Three of five assertions were tautologies of the loop above them — the inner `die` returns `never`, so `unrouted` could not be non-empty. **No tree state reaches them.** Deleted; the success line states what `owed` proves, with its **changed** quantifier, and counts the one permitted subtraction | every printed number is measured, and each `die()` is reachable under a plant |
| ✅ **S1.11.6.2** | **`neighbour`, the fifth origin.** All four buckets were predicates on *changed*, so a two-producer disagreement was **half-routed by construction**. The pair is **siblings through a common consumer**, so one hop is not enough — measured at pass 4's endpoints: +101 consumers *(the two money screens)*, +155 siblings *(`A-F4`'s producer)*. ⚠️ Cost stated: ~3.7× the changed set | the route emits the CONSUMERS of every changed producer |
| ✅ **S1.11.6.3** | **`D4-7` CLOSED.** The S1 loop routed never-swept surface files; the S0 loop asked only whether they **changed**, so **51** never-swept S0 files were in no lane of any round. Its own origin, because *“report the round split by origin”* is this file's own rule | S0's unswept set appears in a lane |
| ✅ **S1.11.6.4** | **THE MEMORY PROTOCOL SHIPS WITH THE ROUTE.** Generated beside the manifests every run, so it arrives with the work rather than in a doc the next round may not open. ⛔ **An OOM is a FINDING, never a retry** | the protocol is in the script, not in a doc a future session may not read |
| ▶ **S1.11.6.5** | **VERIFY + NET.** Regenerate the route and confirm the three known misses come back **routed** · `typecheck` · `lint:rn` · the suites · commit | `A-F4`'s producer and C's two money screens are routed |

**Exit (S1.11.6):** the route reproduces `A-F4`'s unrouted producer and C's two unrouted money screens as
**routed**, S0 is in a lane, and the memory protocol lives in the script.
