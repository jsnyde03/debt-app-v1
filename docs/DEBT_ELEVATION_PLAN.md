...
| order | # | Step |
|---|---|---|
| **1** | **3.5.5.3** | **▶ NEXT — seen-persistence + a replay entry** in More, mirroring the walkthrough's row. The store's `shown` set makes a mark once-per-RUN; this makes it once-EVER, which is what stops it nagging |
