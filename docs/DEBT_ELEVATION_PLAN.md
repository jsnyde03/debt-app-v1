...
**✅✅ Premium-framework audit COMPLETE — CONSENSUS REACHED (round 4, 2026-07-27).** Round 1 (7 lenses → CONDITIONAL PASS) → 2.11.8 fix pass → round 2 (5 verifiers; caught B1 incomplete) → 2.11.10 fix cycle → round 3 (caught the one-time-BNPL chart edge) → round-3 fix → **round 4 = CONSENSUS**. The framework justifies its price; the storefront/wiring/copy fixes hold; the free-tier BNPL payoff math is correct + regression-locked (chart↔date agree across every one-time scenario, proven by algebra + boundary tests). **Full `validate:release:rn` GREEN (e2e 28/28, both themes).** Round-by-round detail → `DEBT_PREMIUM_FRAMEWORK_AUDIT_2026-07-27.md` §ROUND 1–4. Deferred (non-blocking → backlog): R2.8 offline lifetime-mislabel · per-debt amortization cadence · dead `DriftResult` re-export · budget=0 full-horizon chart.

**⚠️ WHOLE-PHASE-2 AFTER-SCAN (2026-07-27) — caught 5 already-shipped BNPL-cadence bugs in the BALANCE paths (the payoff engines + allocator got cadence-scaling; the balance-mutating paths never did).** The phase-scan doing its job (the archetype: catches what per-item scans + the premium-scoped audit miss). **Phase 2 does NOT close until these fold in.**

**▶ 2.13 ⭐ BNPL-cadence balance-path fix cycle (version-necessary).** One shared cadence treatment across all debt-payment-rate + balance-reduction surfaces.
- [ ] **AS.1 [MAJOR]** `projectCurrentBalance` (premium always-current balance, 2.3) — scale the per-month BNPL paydown by the monthly factor (reuse `bnplMonthlyEquivalentMinimum`); today it under-pays + contradicts the trajectory.
- [ ] **AS.2 [MAJOR]** `applyRolloverPayment` (WRITE path via `payday.ts`) — scale the rollover BNPL paydown by the in-window installment count so the persisted balance matches the cash already reserved (`scaleBnplMinimumsForWindow`); today a monthly-paid user's biweekly BNPL balance drifts high every cycle. Touches the `minimumPaidThisCycle` flag + `syncBnplRemaining` "X of N" — approach with care.
- [ ] **AS.3–AS.5 [MINOR]** forecast state-threading (`buildMultiCycleTimeline` thread by the same in-window count it reserves) · Recovery essentials (`recoverySelectors` window-scale the BNPL) · amortization sheet (`selectDebtAmortization` — the backlogged gap, now bumped: `bnplMonthlyEquivalentMinimum`).
- [ ] **AS verify** — multi-cadence + multi-debt asserts on each path · full `validate:release:rn` · then a focused re-audit for consensus.
- **Exit:** balance paths cadence-consistent (all surfaces agree for a cross-cadence BNPL) + verified → **PHASE 2 CLOSES** → Phase 3 opens with the ⭐ Best-in-Class Enhancement audit.
