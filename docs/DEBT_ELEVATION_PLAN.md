...
| **S1.13.7.10.2** | **ASSERTIONS THAT NEVER EXECUTE** — `D1-5` *(`testAllocationsAppearAfterExpensesAndDebts` runs zero assertions)*, `A1-9` *(the What-If test's only assertion is satisfied BEFORE the What-If is opened)*, plus whatever `.10.1` adds | each made to red on the defect it names, with a checked assertion count |
