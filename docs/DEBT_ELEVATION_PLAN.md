...
- **✅ 3.5.8.9 CLOSED 2026-08-07 (cycle 10).** The app marks its own paint moment, the pipeline finds it, `LAUNCH_ALLOWANCE` is gone, and **every beat's first frame is painted** — the check cycles 8 and 9 failed. ⚡ **It also corrected cycle 8's reading:** the empty charts are an 850ms *designed reveal*, and cycle 8's frozen version was a starved runner never starting it. **→ that is the evidence 3.5.8.8 lacked, and it points at the device.**
- **🎯 3.5.8 IS NOW GENUINELY AWAITING JASON:** watch `app-preview-6.9in.mp4` from run 31211254519, then 3.5.8.6b + 3.5.8.8.
