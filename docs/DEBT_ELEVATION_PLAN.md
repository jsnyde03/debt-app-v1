...
| **4.1.9b** | ⭐ **Extract the simulator build into a composite action, and key the `.app` cache on THAT** *(🎯 approved 2026-08-14)*. ⛔ The key currently hashes the WHOLE of `native-e2e.yml`, so the **flow list** busts the binary — both of today's runs paid ~771s of compile for pure YAML edits. Resolves three filed items at once: the flow-list bust · the ~60-line recipe **duplicated** in `app-preview.yml` · and that file **lacking the `.app` cache** entirely. ⚠️ It LOOSENS a key 4.1.3a hardened deliberately — the composite holds every build flag, so only the `maestro test` invocations stop busting it, and those cannot affect a compiled app. **After the current run lands** | |
