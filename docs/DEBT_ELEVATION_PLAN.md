...
- **Active item:** **▶ 2.7 BNPL as a first-class obligation** — 2.7.1 gate ✅ · 2.7.2 model ✅ · 2.7.3 capture/display ✅ · **2.7.4 Guardian-aware cadence ✅ engine (2026-07-24, e2e 19/19)**. **▶ NEXT = 2.7.4.3 surfacing [propose to Jason] then 2.7.5 BNPL calendar** (the last 2.7 piece). **✅✅ GUARDIAN COMPLETE + AUDIT PASSED. ✅ 2.6 · 2.5 · 2.4 COMPLETE**.
...
- [x] **2.7.4 ✅ engine (2026-07-24)** Guardian-aware cadence — bounded BNPL-scoped fix (Jason's call over a general allocator refactor): the allocator under-counted a sub-cycle BNPL (a monthly earner's biweekly BNPL charges 2–3× but was counted once → crunch under-detected). Pure `bnplInstallmentsInWindow` + `scaleBnplMinimumForWindow` scale the BNPL's effective per-cycle minimum to its in-window installment count, wired at `buildAllocation` (card) + `buildMultiCycleTimeline` (lookahead, transient copy); paid-flags/rollover untouched, aligned cadences unaffected. +16 asserts (core window + app integration: monthly-paid reserves $300 not $100). Full gate + e2e 19/19 green. Detail → LOG.
  - [ ] **2.7.4.3 [DESIGN, propose to Jason]** the crunch is already surfaced (the Guardian reads tight on the corrected numbers); a dedicated "your {provider} installment lands {date}, between paychecks" callout NAMING the cause is design-shaped — bring options + rec. _(deferred general undercount + BNPL payoff-rate → backlog.)_
