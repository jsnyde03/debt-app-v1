...
| 3.5.8.3 | **The launch seam** — prove `simctl openurl booted "debtplannerrn:///demo?capture=1"` reaches the arc cold. The scheme and the `capture` param both exist; this is verification, not construction |
...
| 3.5.8.7 | **Verify the FIRST frame of every beat** from the recording itself — Skia painted, numbers right. An empty chart in a store video is worse than no video. **+ carried from 3.5.8.2:** the solved bills carry cents (`$80.63`) that read computed, not real — below the fold at phone size, so this is the step that sees whether they ever reach frame |
