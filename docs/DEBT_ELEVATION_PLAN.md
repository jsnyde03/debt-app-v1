...
- **⚠️ A0 · "View Payoff Schedule" is dead on device — REDESIGN (ship-blocker, Jason 2026-07-30, 3rd report).** Two fixes have already landed and BOTH are in the `c050173` device build Jason tested, so this is not a patch problem — the nesting pattern itself has to go.
  ...
  - **A0.2.1** new route `app/schedule/[id].tsx` (the app's routes are flat — `history`, `cushion-forecast`, `paywall` — so this is the first param route); move the `AmortizationSheet` body into it and drop the sheet chrome.
  - **A0.2.2** entry on the debt row via `ListRow` (it already carries `onPress`/`onDelete`/`onLogPayment`, so a schedule action is an existing seam, not a new pattern) → `router.push`.
  - **A0.2.3** remove `headerAction` + `showSchedule` from `DebtSheet`, then **delete the `overlay` escape hatch from `AnimatedSheet` entirely** — `AmortizationSheet` is its ONLY consumer, so this removes the sibling-of-a-Modal failure class from the codebase rather than just this one instance.
  - **A0.2.4** iPad: in Money master-detail the schedule belongs in the DETAIL PANE, not a push — reconcile with the `inline` path (`3.6.2`) so both form factors stay first-class.
  - **A0.3** close the test gap: Maestro `02-sheet-native-tap.yaml` only taps ✕ on the **Add** sheet, where `headerAction` never renders (it is `isEdit`-only). Add a flow that reaches the schedule and asserts it is visible — the iOS Simulator has real native Modal/navigation semantics, so this class becomes catchable without physical hardware. Update the web e2e for the new entry point.
  - **A0.4** re-verify on the next device build (Phase 6).
