...
- [ ] **2.6.1 [DESIGN GATE]** Switch-in before-scan + design alignment — verify the current shortfall/paused/crunch surfaces vs code (what already routes where), then agree the Recovery Plan UX with Jason: Cover-now · Safe-to-defer (ranked) · gap math · honest un-closeable branch · one-tap apply · the essential-vs-deferrable model. Decompose 2.6.2+ to leaves out of that agreement.
- [ ] **2.6.2** Essential-vs-deferrable heuristic-v1 (pure, tested) — classify obligations + a user override; reconciliation-tested.
- [ ] **2.6.3** Recovery engine (pure, tested) — build the catch-up plan: cover-now set · safe-to-defer ranking · gap math · the honest "can't close it this cycle" branch.
- [ ] **2.6.4** Route every trouble surface (shortfall · paused · looming-crunch) into the Recovery Plan; one entry point.
- [ ] **2.6.5** One-tap apply + surface the Interest-Saved counterfactual.
