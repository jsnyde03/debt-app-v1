...
| ▶ **.5.6** | **`C3-14` — *"saved — one target could not be read"* hard-codes **one**.** ① switch-in: count the unread targets the caption stands for, and whether any other caption spells a count the same way · ② the count from the same list the caption is shown for · ③ asserted at 1 and 2, planted · ④ close, push | the caption counts the targets |
| **.5.7** | **BOUNDARY + LEDGER DRAIN** — ⚠️ first: `native-e2e` is red on 2 of 9 iPhone flows *(stale label · a coach mark that does not survive a segment switch)*; repair and one dispatch | 52/52 read from its own summary line · both suites · typecheck 0 · stale drained · pushed |
