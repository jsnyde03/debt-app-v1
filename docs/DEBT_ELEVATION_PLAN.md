...
- **3.5.4 — bounded demo (B) + GTM funnel ▶ NEXT ACTIVE BUILD [blocker/major-fix].** 🔑 Switch-in reads the 8-item entry list in `DEBT_TUTORIAL_AUDIT_R8_LENS_REPORTS.md` §LENS C — it is the spec. **Decomposed:**
  ...
  | 3.5.4.3 | ✅ **DONE 2026-08-06** — `ExampleCanvasMarker`, keyed on `isSandboxStore` and mounted by `Screen` itself, so every surface a demo reaches is marked without per-screen wiring. Above the scroller (asserted by position), `header` role, spoken once on entry, withheld during a walkthrough so it can't double. Log: 3.5.4.3 | ⚙️ |
  | 3.5.4.5 | ✅ **CANCELLED 2026-08-06 — [D18] answered it.** A kiosk fences navigation, so `/more` and `/paywall` are unreachable with a demo mounted and the guard's premise holds. It stays STRICT for both runs; no `strict` flag. The false "owed at 3.5.4" claim was deleted from `StoreContext` per [D17]. Log: 3.5.4.1 | ⚙️ |
  | 3.5.4.6 | ⚙️ **ENGINE DONE 2026-08-06 (`c05139d`)** — `demoRun` walks clear→tight→at-risk on a timed script (the at-risk stage IS the free/Recovery contrast); day-one bound asserted per stage, verified red. **Remaining: the watched-run CHROME + copy** — pacing/skip affordance and the words, which are 🎯-facing and pair naturally with .7's entries | ⚙️ + 🎯 |
  | 3.5.4.4 | ✅ **DONE with .6** — reordered so the API had a real second caller. Narrower than specced: `stage(index)` is beat-shaped and the demo has no beats, so only the **timer registry** moved (`sandboxRun`). Its argument was present, not speculative — one global background handler cancels every run's timers by construction. Log: 3.5.4.6 | ⚙️ |
  | 3.5.4.8 | ✅ **DONE 2026-08-06 (`f4c875e`)** — `demoSeed` deleted; the Welcome entry REPOINTED at the sandbox demo (not removed) behind one shared `isDemoReachable()`; the `isDemoMode` gate removed from payday capture. **The persisted FIELD stays inert** — dropping a key is a schema change and belongs to Phase 5's migration, not a feature commit. Log: 3.5.4.8 | ⚙️ |
  | 3.5.4.7 | ✅ **DONE 2026-08-06 (`dc5b4d1`, `4a05b05`)** — paywall "See it in action" + repointed Welcome entry, both behind one `isDemoReachable()`; terminal exits via `exitDemo`; the demo's own dock. **A real user gets the demo in v1.7** (🎯 2026-08-06). Caught a broken conversion path: `/paywall` sat inside the onboarding guard, so ending the demo dumped a pre-purchase viewer into onboarding. Log: 3.5.4.7 | ⚙️ + 🎯 |
  | 3.5.4.9 | ✅ **DONE 2026-08-06 (`91a5c30`)** — eight events, **no financial data by construction** (closed union of literals), **no sink attached** (Phase 6's privacy audit decides that), opt-out honoured at the choke point with a switch in More. Log: 3.5.4.9 | ⚙️ |
  | 3.5.4.10 | ✅ **WEB-VERIFIED 2026-08-06 (`c0d5966`)** — both themes by looking, `/demo` added to the axe scanner, 129/129 e2e. Found two defects no test could: a doubled disclosure and the dock clipping the tab bar. ⏳ **Native lane owed** — the dock, the hidden tab bar and the route guard are all native-sensitive | ⚙️ |
...
- **3.5.5 — feature-discovery coach-marks (C):** calm · one-at-a-time · dismissible · replayable · iOS-16-safe · rendered OUTSIDE gesture handlers · platform-gated · over the ENUMERATED hidden-gesture inventory (long-press menu · Cash-Runway scrub · Can-I-Afford · income-varies toggle · swipe-to-delete · chart scrub · Log-payment · widget/Lock-Screen/Siri).
  ...
  > 1. **Scan a statement (§2.8)** — on-device OCR is a real differentiator hiding behind an unremarkable dashed row on Money.
  > 2. **What-If simulator (§2.2)** — premium, and DISTINCT from the "chart scrub" already listed.
  > 3. **⚠️ The payoff schedule** — **3.7.A0 MOVED it this session** (edit-sheet header → row long-press menu + a sheet body row). Anyone who knew the old location loses it, and new users have no way to find the menu. Relocating a feature without adding it to the discovery layer is how it goes dark.
...
> **🔍 [AUDIT GATE] 3.5.8 readiness review — scheduled for RIGHT AFTER 3.5.7 (Jason 2026-07-30).** A light agent pass (not Fable 5), deliberately not run early: 3.5.8's prerequisites don't exist until 3.5.4/3.5.7 land, so an early review would judge the plan against a repo missing them. **Questions already identified, so it starts warm — verify each against the REPO, not the plan's wording:**
> 1. ✅ **ANSWERED (2026-08-05, 3.5.6b).** It exists in THIS repo — `.github/workflows/native-e2e.yml` + `apps/rn/.maestro/`, six flows, and 3.5.6b added the two walkthrough ones.
> 2. ✅ **ANSWERED — yes, and it is proven.** Prebuild → unsigned Simulator compile → boot → install → Maestro, ~35 min end to end on a GH macOS runner, no Codemagic and no signing. What was "likely the biggest gap" is now the working substrate the capture pipeline stands on.
> 3. **The launch-into-demo seam:** how does CI launch straight into the scripted demo deterministically (deep link · `QA_TOOLS` · env)? Probably must be ADDED by 3.5.8.
> 4. **Sequencing:** 3.5.8 sits after its true dependencies (3.5.4 demo, 3.5.1 triggers)?
> 5. **Contradiction to resolve:** the demo is specced as a "~5s hook" but Apple requires **15–30s** — does the capture need its own longer cut?
> 6. Practicalities: ffmpeg preinstalled on GH macOS or must be added · guaranteeing store-exact resolution · forcing DARK mode · where the artifact lands and how Jason retrieves it · confirm a sim build needs no signing.
> 7. **⭐ RESEARCH THE BEST CAPTURE TOOLING (Jason 2026-07-30) — Maestro is the incumbent, NOT the assumption.** Maestro is already in the repo for native e2e, which makes it convenient, but convenient ≠ best for a *premium marketing asset*. Survey the current field and recommend with reasons: **Maestro** (own recording commands?) · raw **`xcrun simctl io recordVideo`** driven by a plain script · **XCUITest/XCTest UI recording** (native, most faithful) · **Detox** · **Appium** · purpose-built App-Preview tools (e.g. the RecordIt/Rotato/Screen-Studio class) · real-device capture via **QuickTime/Xcode**. Judge on: **visual fidelity** (does it render Skia · `expo-blur` UIKit material · the finale mesh-gradient faithfully?), **frame-rate stability** (dropped frames wreck a preview), **determinism/repeatability**, **runs headless in CI on a free GH-macOS runner**, exact store resolution, and setup cost. Web-search current-year guidance — do not answer from memory. A better tool is worth switching to here even though Maestro is already wired.
>
> **⭐ 3.5.8 — the capture is AUTOMATED, output = the actual submittable video (Jason 2026-07-30).** Target: one command produces the file, with no Mac in Jason's hands. **Pipeline (GH-macOS runner, the one already carrying the Maestro layer):** boot the largest-iPhone simulator → install the sim build → launch straight into the 3.5.4 demo scenario (deterministic seed, DARK) → `xcrun simctl io booted recordVideo` → **Maestro drives the scripted beats with deliberate pacing** (the same script as the in-app demo) → stop → `ffmpeg` to constant 30fps, trimmed to the 15–30s window → upload as a CI artifact Jason drops into ASC.
> **Apple rules verified 2026-07-30 (not from memory):** simulator capture via `xcrun simctl io booted recordVideo` is legitimate and yields exact store pixel dimensions · **15–30s** · **constant 30fps** · **<500MB** · in-app footage only · one video at the largest size per device family scales to the rest (currently 6.9″ iPhone — re-verify the size at capture time).
> **⚠️ The honest risk:** the simulator may not render Skia, `expo-blur` UIKit material, or the finale mesh-gradient faithfully, and recording can drop frames — and this is a PREMIUM marketing asset. So treat auto-capture as the **draft/iteration** path (free retakes, exact timing, deterministic re-shoots after any UI change) and keep a **real-device re-run of the same script** as the fallback for the submitted asset. Decide on the evidence, after seeing the first capture.
>
> **⭐ Why the App-Preview capture gets cheap (Jason asked 2026-07-30; recorded so it isn't rediscovered at submit).** The 3.5.0 substrate already supplies what a repeatable capture needs: **determinism** (frozen base date + byte-identical builds → a retake after a UI tweak shows the SAME numbers and dates, instead of drifting every run) · **named states on demand** (clear/tight/at-risk are one call each = the storyboard's visual beats) · **a scripted driver** (`advanceSandboxCycle`/`runBeats` walks payday→surprise-absorbed→release programmatically) · **isolation** (record on a device carrying a real plan without touching or exposing it) · a **persona** seed that's realistic but nobody's actual money, with locale-neutral copy already required. Two harnesses can drive it: the Playwright visual script (both themes × both layouts) and the Maestro iOS-Simulator flows. **Aim for ONE script serving three uses — in-app demo · marketing-site embed (3.5.7) · App Preview.** Remaining real work at submit = the video capture/edit itself, and **verifying Apple's CURRENT App-Preview technical rules** (device capture, resolution, duration) per [[feedback_verify_asc_against_current_year]] — never from memory. Capture in DARK mode per [[feedback_dark_mode_screenshots]].
...
- **§3.5.3 the Guardian WALKTHROUGH (added 2026-08-04 by the round-4 device lens).** ⚠️ This whole feature was verified almost entirely by a WEB export; none of the below is observable there, and until now none of it was written down anywhere the device pass would actually be run from.
  - **VoiceOver end-to-end, all 7 beats** — incl. whether activation under the scrim's hit bands is action-dispatched or touch-synthesised (decides whether the a11y fence is complete), announcement delivery when `Motion key={position}` remounts the dock each beat (iOS VO can clip an in-flight announcement on focus churn), and the rotor. `announceForAccessibility` is a literal no-op in react-native-web — **zero** web observability.
  - **Haptic WEIGHTS** — the light per-beat tick vs the medium at the two caused-it moments. Neither web nor the simulator can feel them.
  - **Scrim band animation cost** — four `useAnimatedStyle`s driving `top/height/left/width` (layout props, not transforms) = a shadow-tree commit + Yoga relayout per frame, per band. Fine on Fabric in principle; the question is dropped frames on ProMotion and on older devices. Web writes inline styles and proves geometry only, never frame rate.
  - **Slider gesture arbitration on device** — `Pan.onBegin` sets the value on touch-DOWN with no `activeOffsetX`, inside `FormSheet`'s vertical ScrollView. A scroll attempt starting on the 44pt strip jumps the value, then races the native scroll. Web has no competing native scroll gesture and Playwright never drags. Mitigation if it misbehaves: `activeOffsetX(±5)` + move the set to `onStart`/tap-end.
  - **`measureInWindow` coordinate space on Android** — status bar / edge-to-edge can offset window coords vs the stage band built from `insets.top` + window height, systematically parking subjects high or low.
