...
| # | sub-step | exit line |
|---|---|---|
| **S1.11.5.1** | **THE ENGINE BLOCKER — `A-F3`** *(`windowStartISO` accepted and never read: **$1,200** stated against a true **$300**)* | the parameter is READ, and a test drives the window from both ends |
| **S1.11.5.2** | **THE TIMING WINDOW — `C4-6`, which is `C-7b`'s open half.** `openRestoreConfirm` sets `preview = null` and renders the confirm SYNCHRONOUSLY, then starts the read — so for the whole read the confirm is byte-identical to the un-fixed state, and the button is not disabled because `previewRestore` never sets `busy` | the confirm cannot be committed while the disclosure is unknown |
| **S1.11.5.3** | **THE LEDGERS THAT CANNOT SEE THEIR OWN CLASS — `C4-3`** *(`LIVENESS_RE` reads `balance > 0` only; 7 store-debt sites live on `<= 0`)* **· `C4-4`** *(check 3 `continue`s on a file that MENTIONS `trustSelectors`, which is the escape `C4-7` walked through)* | the ledger's population is derived from the CONDITION, never from one spelling of it |
