...
| ▶ **.5.4f** | **`C3-5` — the Live Activity stamped its belief about the Lock Screen on the ATTEMPT** *(folds `C3-6` + the read-once `areActivitiesEnabled`)*. ⛔ **Switch-in: the finding's remedy is a false fix** — Swift's `start`/`update` return before ActivityKit runs, so a JS `boolean` could only ever see a missing module. ✅ **Done:** ① switch-in · ② Swift `AsyncFunction` → `Bool`, bridge `Promise<boolean>`, web `true` · ③ the manager stamps only what landed — 27 assertions, **10 plants** red for their own reason *(the committed manager reproduces "Looks clear" over "$2,500 short")*. **Remaining:** ④ the `.app` cache key hashes `apps/rn/modules/**` *(it did not: a Swift change restored a stale binary)*; `native-e2e` compiles it · ⑤ plants per claim, device row, close, push. Detail → log | the Lock Screen matches the store after a refused start, update or end — planted each way; the Swift compiles in CI |
