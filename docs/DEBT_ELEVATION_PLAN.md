...
- [ ] **2.11.3** wire `react-native-purchases` to the **EXISTING Debt RevenueCat project** (API key `appl_XUWODZnbbJFPbdMTgBTyKNAGGyp` · entitlement **`premium`** · the live monthly product — reuse, DON'T create a new project, so v1.6 subs restore) — port Gig's client/`PremiumContext`/`legal` STRUCTURE, Debt's config values. Add the native dep → pre-commit native-build pass + Codemagic-glob/plugin check; device-QA @ Phase 6. **Create the NEW Annual $29.99 + Lifetime $79.99 products in ASC + add to the offering.**
