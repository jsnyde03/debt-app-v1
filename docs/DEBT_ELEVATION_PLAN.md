...
| ✅ **S1.12.6** | **[DECISION] ANSWERED 🎯 2026-08-30 — NO. *"Coverage is what I want. Not unneeded files."*** The seed is **not** widened. ⚡ **Measured:** the 393-file route is **212 money/app · 72 tests · 38 instruments · and 71 that carry no money claim** *(theme/motion/UI primitives 34 · demo/sandbox/tutorial 24 · shot fixtures 8 · **5 on the legacy root surface 5.5.1 DELETES**)* — **0 of those 71 were read by any lane.** ⛔ **The gap is 121 UNREAD money files, not 82 unrouted ones.** Pass 6 is scoped by that: prune what cannot carry a money claim, and make per-file coverage a **checkable exit**, not lane prose | ANSWERED |
| **S1.12.7** | ⛔ **[DECISION] 🎯 — should the forecast ledger SHOW a negative running cash?** It clamps at `$0`, so a user $800 short sees three identical `$0` rows and a screen-reader hears *"Pay Phone, −$200, balance $0"*. ⚠️ The clamp is deliberate and asserted — but `formatCurrency.ts:21` states this codebase's own rule, *"if it can go negative, show it"*, and `carriedBalance` next door is deliberately un-clamped for that reason. **Recommendation: SHOW it** — the app already knows the true number and its own doctrine says not to hide money. Cost: two components and a `< 100` colour threshold | a yes/no, recorded here |
| ▶ **S1.12.5.9** | **THE NET.** typecheck ×4 · `lint:rn` **42/42** · three unit suites — all green. ▶ Full e2e + embed running. ⛔ **No `gate:record`** — [D74] writes it at convergence, and pass 5 did not converge | net green and pushed |

| **S1.12.8** | ⛔ **[DECISION] 🎯 — should a null `onboardedAt` credit the reserve for ALL prior surprises, or none?** `?? ''` widens the filter to the entire surprise log — the pre-MF.5 defect — and a null date is the ORDINARY state for restored / v1.6-upgraded users *(two producers of "this user has started", only one stamps a date)*. ⚠️ **A fail-closed fix was written and REVERTED**: it breaks *"a surprise during the hold → tapped"*, whose fixture has no `onboardedAt` and a genuine in-window surprise. **The honest lower bound is when the RESERVE was held, and that field does not exist.** Bounded by `Math.min(surpriseSum, heldReserve)` either way | a yes/no, or a new field, recorded here |
