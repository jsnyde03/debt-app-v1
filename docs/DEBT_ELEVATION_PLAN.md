...
fix and reproduces `A3-4`'s defect mirrored.** Measured — one weekly debt, minimum $50, on a $500 paycheck
that covers everything: `totalRequired = $1,250`, **`shortfall = $750`**, printed on the Guardian card, the
Live Activity, the widget and the paywall lead beside a $250 "Extra payment" row. Monthly control clean.

| # | step | exit line |
|---|---|---|
| **.4.1** | **VERIFY THE PREMISE FIRST** — re-derive the double-scaling against the current code and re-measure the $750 on a fresh fixture. ⚠️ Class membership is a hypothesis, and this class was written before four rounds of edits | the two call sites named, and the printed figure reproduced or corrected |
| **.4.2** | **FIXTURES FIRST, BEFORE ANY FIX** — `minimumPayment: 0` is why **five** instruments in this class are blind (`A3-7` `A3-12` `A3-14` `A2-2` `A3-2`). Give them non-zero minimums and watch which ones start failing | the count of instruments that RED once their fixtures can see the defect |
| **.4.3** | **ONE OWNER for the in-window minimum** — `A2-1`/`A3-1`. Delete the second application, not the first | a single producer, named, with both former call sites routed through it |
| **.4.4** | **`A3-4` — the carried total**: `essentials`, `net`, `carriedBalance`, the cash-runway receipt and the crunch detector all run on the inflated figure | the receipt's rows add up to the totals printed beside them |
| **.4.5** | **THE VACUOUS CONTROLS** — `A2-2` varies alignment not type; `A2-8` pins the UN-FIXED behaviour as the requirement; `A3-2` runs a path production never takes; `A3-14`'s fixture is due five months outside the window; `A3-12` passes `debts: []` to all 28 pairs | each control fails when the defect is planted, proven by planting |
| **.4.6** | **THE ASSERTION ITERATES cadence × debt-type**, never a named member — the standing rule this class exists to enforce | one assertion, iterating, on non-zero fixtures |
| **.4.7** | **THE TWO MINORS** — `A2-3` a caption fix that iterated to fallback BNPLs and not to the plain debts it was widened for; `A2-4` a safety argument that is a carried premise and measurably false | each re-checked against the code, then fixed or filed |
| **.4.8** | **BOUNDARY + `[D79]` re-audit** — full `lint:rn` and both suites, then a FRESH agent, cumulative, with guards derived from **the diff** | 0 new defects attributable to this class's fixes |
