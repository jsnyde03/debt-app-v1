...
- [x] **2.8.3 ✅ built (2026-07-25, device-QA @ Phase 6)** native Apple Vision — a local Expo module `modules/scan-vision` (Swift `VNDocumentCameraViewController` scan → `VNRecognizeTextRequest` OCR → joined text; all Apple, on-device, NO new npm dep) + JS `src/lib/scan.ts` (`scanStatement()`/`isScanAvailable()`) with a `scan.web.ts` no-op split; `NSCameraUsageDescription` added (⚠️ Info.plist, NOT a signing cap → no profile regen; new native module → autolink verified only on a real build). tsc + lint green. **⚠️ native compile/OCR NOT web-verifiable → Phase-6 device-QA.**
