...
- **▶ Next action: 3.5.8 — the App-Preview capture pipeline.** Start it FRESH: its own plan entry requires researching the capture tooling against current-year guidance before assuming Maestro, and that deserves a clean read. The demo it captures is built and reviewed (3.5.4.11). **Carry in two open items** from the audit doc's capture section, and its hard requirement: hold each beat long enough for Skia to paint, and verify the FIRST frame of every beat — an empty chart in a store video is worse than no video.
