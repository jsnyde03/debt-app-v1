...
| **1** | **3.5.5.4** | **▶ ACTIVE — inventory VERIFIED, built, ⚠️ parked NOT-GREEN on branch `3.5.5.4-inventory`.** 🎯 **3 marks** (payoff-schedule · row long-press, iOS-gated · trajectory scrub); 3 "hidden" entries were plainly visible and widget/Siri have no on-screen subject to point at. ⚠️ **Found + fixed a 3.7.A9-class defect: `LogPaymentSheet` was reachable ONLY via the iOS long-press** (none on Android/web) → a visible row in the debt sheet. **Next: bisect the .4 diff** — the gate fails one tutorial test under load; a control at .3 gates 139/139. Log: 3.5.5.4 |
| 3 | 3.5.5.6 | **Verify + close** — both themes · a11y (a mark must not fence the control it points at) · e2e · native lane. ⚠️ **Owed to the device lane by .5:** the in-sheet layer exists *because* a root overlay renders behind a presented Modal, and the web cannot see that — the mark's device layering is unproven until Maestro runs it |
