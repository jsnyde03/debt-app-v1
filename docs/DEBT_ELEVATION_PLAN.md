...
| ✅ | **P6.4.2 DONE 2026-08-20.** The claims cluster was **empty** *(L3-6/L3-7 built at T5.2, L3-5 dies with P6.11)*, so the slot went to what the triage found: **`lint:money` was GREEN over FIVE hand-rolled money sites** — and one, `Slider.tsx`, was handing VoiceOver **"$5000"** with no separator, byte-for-byte the defect that justified the gate. ⚡ **My own enumeration went 3 → 4 → 5** in one sitting. **A regex cannot see the JSX form** — `$` + `{expr}` is byte-identical to a template interpolation — so the gate gained a **TS AST pass**, a broadened shape 1 and a `.replace('$')` shape; both new detectors mutation-planted. e2e **207 → 208**. Detail → log |
| ✅ | **P6.4.3 DONE 2026-08-20.** The number-formatting cluster was **empty** — L4-5/7/9 closed at T6.5/T6.7, **L4-8 DECLINED on the record**, L4-10 closed by its own "no fix needed". What survived was structural: **L4-15** → `selectLivingReserveRequest`, one owner for the everyday request *(the expression was verbatim in `money.tsx` and `living-expenses.tsx`)*, pinned **against the engine's own figure** rather than a literal so selector and engine cannot drift; mutation-planted (drop the `enabled` filter → red, `420` vs `1419`). **L4-14** → the docstring names Today only and records 3.6.4 as the reason Progress diverges. ⚠️ `MasterDetail` looked like a second consumer on a grep and is **a comment reference, not an import** — checked before writing the replacement |
