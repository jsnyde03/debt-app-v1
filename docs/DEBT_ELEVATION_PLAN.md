...
| ▶ **.5.4b** | **`C3-11` — THE CUSHION FORECAST ASKS NO TRUST QUESTION.** Switch-in measured: `'solved-projection'` is **exact** for the forecast's own figures *(0 over · 0 holes — once the shapes can clear a debt and fill a goal inside the horizon)*, while `'required-plan'`, the claim Today's card asks, has **7 holes** against them. The scorecard is stored history and is not gagged. Detail → log | the runway withholds exactly when a figure it draws moves; the scorecard survives |
| ✅ **.5.4b.1** | **SWITCH-IN DONE 2026-09-13** — premises re-measured; ⚠️ the first probe's shapes could neither clear a debt nor fill a goal in-horizon, so APR and the saved amount read as over-suppression. Detail → log | ✅ |
| ✅ **.5.4b.2** | **DONE 2026-09-13** — eight shapes, asserted per SURFACE; eight plants `MATCHED`. ⚠️ Plant G's premise was wrong *(the family contains the runway)*, so the forecast surface can only catch over-suppression — proven by a constant figure. Detail → log | ✅ |
| ✅ **.5.4b.3** | **DONE 2026-09-13** — the runway withholds on `'solved-projection'` behind *"Your forecast is on hold"*, naming the figure; the scorecard stays. `lint:copy` · typecheck 0 · floor 2→3. Detail → log | ✅ |
| **.5.4b.4** | e2e — a lost minimum withholds the runway; a lost `typicalAmount` keeps it | collected by name |
| **.5.4b.5** | Plants — no guard · the wide claim · `'required-plan'` | each reds for its planted reason |
