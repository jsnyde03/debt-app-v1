...
| ▶ **.5.4g** | **`C3-1` — Siri's log-a-payment list says `$0` for the one debt the app refuses to state a balance for.** ⚡ The in-app twin of this flow was fixed in pass 5 (`logPaymentSubtitle`, *"balance not read"*); Siri was not. ① switch-in, premises measured · ② one owner for the balance phrase in `logPaymentCopy.ts`, called by the sheet and `snapshot.ts`'s `debtsJson` · ③ asserted by name over an unread balance, a whole-row loss and a readable control; planted · ④ close, push | Siri's row names the unread balance in the sheet's own words; a readable debt still states its figure |
