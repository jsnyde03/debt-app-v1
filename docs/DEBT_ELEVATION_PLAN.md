...
**▶ 2.5 Smart obligation quality layer** — **RESCOPED at switch-in (before-scan 2026-07-24, Jason ✓).** The pre-authored "add 4 capture fields" premise had drifted: **category · BNPL-term (`Debt.remainingPayments`/`scheduledPaymentAmount`) · amount-variance (`RequiredExpense.expenseType`) ALL already captured**, and **one-off is already fully handled** by `one-time` recurrence (dropped in rollover + timeline) → **one-off heuristic SCRAPPED.** A second, deeper read (2026-07-24) narrowed the real gap further: **BNPL-expiry is ALSO effectively already handled** — `DebtSheet` requires `minimumPayment>0`+`balance>0`, and `applyRolloverPayment` pays BNPL down interest-free to balance→0, so a finite BNPL retires on schedule (residual edge is minor + conservative); `remainingPayments` is amortization-display-only. **→ BNPL-expiry SCRAPPED too.** So 2.5 lands on **two genuine gaps: ① trials (unmodeled) + ② variable-bill reserve (flag captured, Guardian never consumes it).** _Structure-first; trial capture-UX aligns w/ Jason at 2.5.2._
- [x] **2.5.1** Schema — trial/intro fields on `RequiredExpense` (isTrial · fullAmount · fullChargeDate), optional/backfill-safe (no migration). ✅ `08d28a4`.
- [x] **2.5.2** Trial capture UI ✅ — "Free trial or intro price" toggle in `ExpenseSheet` revealing full-price + kick-in-date (free-trial $0-intro validation relaxed); Bills-list meta indicator "· Trial → $X {date}". e2e `trials.spec.ts` (resolver reprices the read: converted→tight, not-yet→clear); **both-theme visual verified** (Bills list). ExpenseSheet modal → device-QA (RN-web can't drive the FormSheet).
- [x] **2.5.3** Core (pure, reconciliation-tested) ✅ — (a) trial→full-price resolver (`obligations/effectiveObligationAmount.ts`) applied at the 2 store→engine boundaries (`08d28a4`); (b) variable-bill buffer @ 15% (`VARIABLE_BILL_BUFFER_FRACTION`), composed into the uncertainty `max` (not stacked on discovery), premium-gated. +variable/combinedHoldback asserts, test:regression + test:app green.
- [x] **2.5.4** Built — engine wiring done (resolver + variable buffer feed allocation/forecast/Guardian). **Today "trial ended — keep it or cancel it?" card** (Jason ✓ Today-card placement): `selectTrialConversion` (fires when `fullChargeDate <= currentDate`, still `isTrial`; tier-AGNOSTIC — a cancelled trial pollutes even the free forecast); Keep it → `updateExpense` sets `amount=fullAmount` + clears trial flags · I cancelled it → `removeExpense` · Not now → ephemeral dismiss. +9 app-layer selector asserts (green); e2e card test added. **⏳ POST-RESTART: confirm card e2e green on fresh dist + both-theme visual.**
- [ ] **2.5.5** Verify — post-restart clean full gate (`validate:release:rn`) + card visual both themes.
- **Exit:** the Guardian consumes trial + variable-variance signals; trial/variable obligations project honestly; accuracy protected; green.
...
- [ ] **2.9 Momentum** — Interest-Saved ledger as the spine (the always-true, un-chattable number); streaks demoted; debts-vanquished archive (hooks the confirmed-$0 signal).
