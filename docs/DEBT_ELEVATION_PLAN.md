...
- **3.5.3 — the tutorial arc (≤7 beats):** **predict-then-reveal open (E3)** → read the bar → **your line (drag the floor → live re-plan crossing clear→tight, haptic detents at each state boundary [E4])** → the reserve (surprise → tight→absorbed→clear via scripted rollover; spatial money-travel, calm register) → **a Recovery glimpse** (one scripted at-risk→safe-to-defer card) → safe move + "your call" → **wrap** (scorecard "once I learn you" + the **kept-promise flag [E7]** + the on-device/honest-moat line) → **E1 HAND-BACK FINALE: crossfade the sandbox into the user's REAL Guardian card (their real first read; free lands on the real free card + invite = the app's best paywall).** Both-theme + a11y verified.
  > **⭐ Settled 2026-07-31 (Jason):** runs **IN SITU over the real Today** (3.5.3.0 rewire done) · **2 interactive beats** (drag-the-floor · confirm-your-bills — [D10] replaced "tap-the-surprise": a
  > surprise is an EVENT the engine records, not a control a user operates; other 5 scripted, so the a11y-operable surface stays at two controls) · a **persistent "Example" marker** on the card, present on EVERY beat — the entry copy alone scrolls away, and by the tight/at-risk beats a real paycheck figure is on screen in a scary state.
  > **+ ONE ORIENTING LINE in the finale** — e.g. "your debts live in Money, your progress in Progress". Closes the "where is everything" gap in a sentence, since the tutorial deliberately never leaves Today. NOT a tour; one line, then hand back.
...
> 1. Does a **GH-Actions macOS workflow** actually exist in THIS repo and already run Maestro (`.github/workflows/`, `apps/rn/.maestro/`; commit `d82fb56` reportedly added it) — or is that Freedom's?
> 2. **Is an installable iOS SIMULATOR build reachable from a GH runner at all?** The iOS build may only happen on Codemagic — likely the biggest gap.
...
- **⚠️ A0 · "View Payoff Schedule" is dead on device — REDESIGN (ship-blocker, Jason 2026-07-30, 3rd report).** Two fixes have already landed and BOTH are in the `c050173` device build Jason tested, so this is not a patch problem — the nesting pattern itself has to go.
  ...
  - **⏳ A0.4 — re-verify on the next device build (Phase 6).** The ONLY step that can actually close this: web has no native Modal, so it structurally cannot prove the fix. _Also re-check the iOS long-press "Payoff schedule" menu item, which has no web equivalent at all._
