...
| ▶ **.5.5** | **`B1-1` — the save plan paced off the partition total while the card printed spendable** *(an $835/paycheck promise out of $675, and that pace is written to the store)*. ① switch-in: re-run the finding's measurement on current code · ② one producer for the pace and the printed figure · ③ a goal already stored with a wrong pace — measured whether it outlives the fix, and repaired or reported · ④ asserted by name with a control; planted · ⑤ close, push | the pace and the printed figure share one producer; the stored goal cannot outlive a wrong one |
