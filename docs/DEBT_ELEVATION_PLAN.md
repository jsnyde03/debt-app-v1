...
- **Active item:** **✅✅ 2.9 Can-I-Afford-This? (inverse Guardian) COMPLETE (2026-07-25, e2e 23/23, both themes; whole-item after-scan done).** **▶ NEXT = 2.11 Revenue spine** — the premium FEATURES are done, so the monetization plumbing is the highest-value next (turns substance into revenue). **✅ 2.8 (native OCR → Phase-6) · ✅✅ 2.7 BNPL · ✅✅ GUARDIAN + AUDIT · ✅ 2.6 · 2.5 · 2.4**.
...
**▶ 2.11 ⭐ Revenue spine (the active build)** — the premium FEATURES are done (Guardian · BNPL · scan · affordability), so the monetization plumbing is the highest-value next: it turns substance into revenue. Port the Gig paywall stack (no IAP wiring exists in the RN app yet) + add Lifetime + portfolio-sub seam + analytics + Sentry. _**Switch-in = 2.11.1 [DECISION] — decide-first, before ANY StoreKit SKU.**_
- [x] **2.11.1 ✅ [DECISION GATE] (2026-07-25)** — locked w/ Jason: **prices = Monthly $4.99 (live) · Annual $29.99 · Lifetime $79.99 (new)**; **Lifetime = on-device Premium forever, EXCLUDES Connected + Ava**; 4-tier ladder holds; privacy CLAIM = **"100% on-device"** (NOT E2EE-backup — Phase 5); **NO free trial — paywall from Day 1** (the generous free tier IS the proof window; avoids the day-13-cancel reflex). Reuse the existing RevenueCat project + `premium` entitlement (v1.6 subs restore).
- [x] **2.11.2 ✅ (2026-07-25)** — before-scan found the per-feature gating apparatus (`PremiumFeature`/`PREMIUM_FEATURES`/`useHasFeature`/`hasFeatureAccess`/`useIsPremium`) **100% unused** (and stale-lying); the app gates uniformly via inline `subscriptionPlan === 'premium'` at 12 sites (pure selectors can't use a hook anyway). Deleted the whole dead `subscription/` dir (Jason ✓ A — refined: no lone orphan hook); documented the inline standard in `models.ts`. tsc · lint · test:app green.
- [x] **2.11.3 ✅ (2026-07-25)** — ported Gig's vendor-agnostic facade to `apps/rn/src/premium/`: `purchases.ts` (pure, web-safe, entitlement `premium`) · `purchasesClient.ts`/`.web.ts` (native `react-native-purchases@^10.4.4` behind a `__DEV__`/iOS/key guard + public Debt key fallback `appl_…`; web stub no-op) · `legal.ts` (Apple EULA + the live Debt privacy/support URLs, centralized — `more.tsx` now imports them) · `premiumSync.ts` `useInitPremium()` wired into `_layout` (drives `subscriptionPlan` off the `premium` entitlement + a listener; detached in dev/web so the Simulate toggle stands, never downgrades on a failed fetch). **Pre-commit native pass: LOW risk** — no standalone `.xcodeproj`, no Expo config plugin; the current `codemagic.yaml` builds Capacitor, so the RN native build (with this dep) is Phase 5/6. tsc · lint · test:app green. **✅ Jason finished ASC + RevenueCat setup (products + offering + `premium` entitlement).** **Local `.storekit` config DEFERRED to device testing** — web/e2e uses the detached-facade static fallback, and ASC sandbox now covers real purchases (not needed to build/verify the paywall).
- [x] **2.11.4 ✅ (2026-07-25)** — `app/paywall.tsx` modal route in the Debt premium visual language (**both themes screenshot-verified**): hero + 6 benefit rows (effort-not-info) + 3-plan vertical layout (Annual $29.99 "Best value" preselected · Lifetime $79.99 "Pay once" · Monthly $4.99), billed price the most prominent element per plan; RevenueCat facade drives packages (static fallback on web/dev), purchase/restore set the plan; Apple 3.1.2 (auto-renew disclosure incl. "lifetime = one-time" · Terms/Privacy links · restore). `paywall.spec.ts` both themes; **full e2e 25/25**; tsc·lint green. Analytics events → 2.11.6. _(Route exists but nothing navigates to it yet → that's 2.11.5.)_
- [ ] **2.11.5** wire the paywall in — existing premium invites (affordability · Guardian · etc.) route to it; `subscriptionPlan` entitlement driven by RevenueCat `entitlements.active["premium"]`; keep the dev "Simulate Premium" toggle for testing.
- [ ] **2.11.6** portfolio-sub seam (cross-app ecosystem entitlement hook) + paywall-funnel analytics + Sentry error reporting.
- [ ] **2.11.7** launch-flag (paywall built but purchase behind a value-gate flag, OFF until Jason flips — [[feedback_dont_launch_paid_tier_until_worth_price]]) + verify: e2e (renders · plan-select · restore) · both themes · documented Apple 3.1.2 checklist.
- **Exit:** revenue spine wired + paywall shipped behind the launch-flag, Apple-compliant, verified; the launch-FLIP stays Jason's value gate. **This closes Phase 2 → the Premium-framework audit gate below.**

### The Phase-2 close gate _(2.1–2.9 shipped · 2.11 revenue spine is the Active Queue above · 2.10 → Phase 3 · 2.12 → Phase 5/6, all Jason 2026-07-25)_
