...
| ✅ **S1.11.4.2** | **THE MOUNT POINTS.** ✅ **`C4-9`** *(blocker)* — the ring credited **$12,000 the user had not paid**, its only trust check sat in the `!hasDebts` branch; four figures suppressed together, e2e now runs the mixed portfolio. ✅ **`C4-7` + `D3-2`'s `PARTIAL`** *(blocker)* — the Lock Screen and Siri refused while Today said *"the spare **$1,800**"* against a true **$300**; the verdict goes with the figures, and `requiredPlanTrust.test.ts` now walks **six surfaces in ONE list**. ✅ **`C4-2` + `C-4`'s `PARTIAL`** *(blocker)* — the trophy shelf filed a card owed IN FULL as *"$12,000 paid off"* and offered it for sharing; **membership**, which every guard had left open. ⛔ The finding's own remedy would have **deleted the row from Money** — `partitionDebts` is total by construction, and two siblings it never named were wrong too. **S1.11.4.2 CLOSED** | every surface that states the claim in ONE list, and the assertion walks the list |
| ✅ **S1.11.4.3** | **THE RESTORE DOORS — CLOSED.** `C4-11`: the ledger said **two** and there are **four**; the launch-time offer replaced a paycheck and a first debt on one tap saying neither side. `describeLocalOverwrite` is the half neither owner had, and **`lint:restore-doors` DERIVES the population** from every production `importStore(` — four plants, incl. a fifth door in a file no list names. ⛔ `C-7b`'s open half is `C4-6` (the null-preview window) and stays in `S1.11.5` | the doors are enumerated once and the test walks that enumeration |
| ✅ **S1.11.4.4** | **THE OFFER/CAPTION PAIRS — CLOSED.** `C4-5`: the caption was a **field of the offer**, and both go `null` together — with ONE pot, and it the unread one, the user was told nothing. Hoisted to a store fact; `ARITIES` walks four members on **both** surfaces. `C4-1` *(blocker, `C-6`'s open half)*: the installment COUNT is not a dollar figure and was gated nowhere — *"0 of 2 paid"* against a true **2 of 4**. `BNPL_COUNT_FIELDS` names the list once, at the producer. ⛔ `C4-1` struck from `S1.11.5` | the caption fires at every arity of the list, not only the two-pot one |
| ✅ **S1.11.4.5** | **TWO PRODUCERS — CLOSED.** `A-F4` *(blocker)*: the DATE and the CHART disagreed on a **2% band** of ordinary credit cards — *"September 2043"* over a curve that never descends. `cannotAmortize` is **one module** now, called at one point in the month; the differential assertion **sweeps the band** instead of naming a plan it happens to agree on. `F-B3`: a whole unreadable LIST was described as *"1 whole row"* above **Replace my data**. ⛔ The `(`-prefix backstop **stays** — replacing it fails OPEN | the producers are asserted to **agree on one shared fixture**, never each against itself |
| ✅ **S1.11.4.6** | **THE SECOND SITE — CLOSED.** `A-F5`: the onboarding APR bound could be deleted with **325/325 e2e and all three unit suites green**; `APR_ENTRY_PATHS` is a list the assertion walks. ⛔ **Two defects in my own first cut** — a contradictory fixture the suite already records, and *"nothing was written"* assertions that were **vacuous by timing**. `G-6`'s open half `D4-10` was **already closed by `S1.11.2`** *(re-measured, not read)*; its real residual — repointing `typecheck:scripts` leaves every assertion about that config vacuous — is now direction 4 | both sites guarded by an assertion that iterates the paths |
| ✅ **S1.11.4.8** | **[DECISION] ANSWERED 🎯 2026-08-28 — the ack SILENCES the card and does not VERIFY the data.** A whole-row / whole-list loss now survives the acknowledgement, so the app stops resuming the debt-free framing over a portfolio it never read. The question stays answerable two ways, both asserted: the list comes back, or the user confirms it *("These are all my debts" → `resolveUnreadableRows`)*. ⛔ A standing control said the two parenthesised losses are **not** the same question — by failing | CLOSED |
