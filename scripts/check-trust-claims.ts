...
const LIVENESS_OPEN: Record<string, { sites: number; why: string }> = {
  ...
  'apps/rn/src/store/payday.ts': { sites: 1, why: '`balance === 0 && !prior` — a snapshot suppressor for a debt that was already at zero, on the RAW value. Unmeasured against a repaired 0. ⚠️ S1.12.10 added a SECOND balance question to this file and it is deliberately NOT counted here: `priorPot > 0` asks "was the expense-reserve pot already held", and the pot is a figure the app computes from its own allocation — there is no unreadable reserve, so no repaired `$0` can hide in it. It is named rather than left inline so the distinction is visible; `MAX_LIVENESS_SITES` is downward-only and its docblock records that it went up exactly once, for an instrument widening and not for a new site' },
