...
const EXEMPT: Record<string, string> = {
  ...
  'lib/joinedCode.ts':
    'NOT A GATE - it is the shared producer `check-finding-guards` and `unreadInputsCopy.test.ts` read through, so it has no population of its own to floor; its consumers floor theirs. It acquired the stripper at round 6 (`V7`), replacing a line-prefix heuristic that missed a block comment opened MID-LINE and therefore read a deleted guard as present. Verified red under a blanked stripper 2026-09-04: lint:finding-guards exit 1 (a blanked producer finds no token, so every guard reports missing) and test:joined-code exit 1 (its own weld and lineAt assertions fail). Both directions are asserted by test:joined-code, which nothing covered before round 6.',
  'check-finding-guards.ts':
    'Not a tree sweep - it reads the ONE file each registry entry names, and the strip is only the `W1` tripwire (refuse a file whose comments survived, rather than answering the guard question over it). Its actual reading goes through lib/joinedCode. Verified red under a blanked stripper 2026-09-04 (exit 1): a blanked producer finds no token, so all 280 guards report missing. The tripwire itself fails SILENT under a blanked stripper - no line then starts with // - which is why this entry is a measurement of the OTHER path rather than of the tripwire.',
